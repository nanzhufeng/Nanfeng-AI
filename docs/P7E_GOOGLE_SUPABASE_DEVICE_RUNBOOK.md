...
4. 先运行 `scripts/p7e-supabase-readiness.sh --readonly --check-auth`，只记录布尔结果。必须确认 `base_sync_migration_contract_present`、`list_sync_migration_contract_present` 和 `all_five_sync_rpc_contracts_present` 都为 `true`；用户提供明确 target 且授权 mutation 后，按顺序部署 **P7-C 基础迁移** `202608130001_p7c_secure_sync.sql`、**P7-F 列表迁移** `202609120002_p7f_list_sync_documents.sql`，再刷新 PostgREST schema。不得只部署列表 RPC。
5. 部署前后以只读核验表存在、force RLS/default deny、direct grants、五个 RPC signature/grants、function JWT policy、anon 拒绝。不要读取密文；经单独授权才使用 synthetic non-sensitive envelope 做 commit→readback revision/hash。
