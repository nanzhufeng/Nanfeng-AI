...
## 2026-09-01：撤销 Android → Desktop“同步完成”结论，深层验收 blocked

- **裁决：** 上一轮只点开了一级菜单、一级弹窗和少量顶层页面，未覆盖 Android live source 的完整深层路由、表单、状态机、回读、返回恢复与重启恢复；因此“全部完成”“全方位同步完成”及 `design-qa.md` 的 `passed` 结论立即作废。当前总目标状态为 **blocked on implementation and deep local acceptance**，不是发布完成。
- **必须补齐的深层范围：** 主界面、抽屉、模型自动／日常／深度及候选／设置、加号各操作、搜索所有分类与每种文件预览／打开／定位／返回、定时任务创建／编辑／暂停／恢复／删除／通知／失败重试、南枫转写图片／PDF与音视频的选择／导入／任务详情／设置／执行／取消／重试／预览／导出／继续对话、设置全部二／三级页与弹窗、会话列表滚动与标题点击、Markdown／附件／费用／错误态。
- **统一验收门槛：** 每条适用流程至少覆盖“入口 → 填写／选择 → 保存／执行 → 回读 → 返回恢复”，并覆盖空态、加载、成功、失败、重试、取消、权限／凭据缺失和重启恢复。Desktop 保留大输入框、常驻侧栏、宽屏双栏及键鼠效率，不机械复制手机尺寸。
- **证据边界：** 先以 Android live source 建立代码级深层页面树与差异矩阵；再构建最新 release bundle，以独立 Bundle ID 和独立 `/tmp` 数据根做原生 Computer Use 逐页点穿。真实 Provider／Google／Supabase／系统通知未获授权时必须单独标为外部待验证，不能用 UI、单测、构建或 localhost mock 冒充。
- **安全边界：** 保护当前约 168 个在途文件，不 reset／clean／stash；永久禁止任何 `connected*AndroidTest`；本轮不连接、安装或操作 OPPO。

## 2026-09-01：Android → Desktop 一级视觉同步（历史结论已撤销）

- **同步原则：** 以 2026-09-01 Android live source 生成的主界面、抽屉、模型、加号、搜索、定时任务、南枫转写和设置截图为视觉基线；同步功能、信息层级、图标语义、卡片语言与状态，不照搬手机尺寸。Desktop 独有的大 Composer、常驻侧栏、宽屏搜索、双栏设置和双栏转写工作台继续保留。
- **落地范围：** 模型根级改为自动／日常／深度并进入完整候选；移除可见 Compare；加号补齐图片／文件／实时网页搜索；侧栏顺序、会话创建日期、列表与正文滚动恢复对齐；搜索补齐多格式预览和打开方式；定时任务独立页面完成；南枫转写增加 GLM-OCR 图片／PDF 默认模式并保留 Qwen 音视频；Assistant Markdown、人民币费用、设置 IA／卡片／图标完成同步。
- **原生视觉证据：** 最新 release bundle 以唯一验收 Bundle ID、独立 `/tmp` 数据根运行，Computer Use 实际打开主界面、模型根级／日常候选、加号、搜索、定时任务、两种转写模式和设置并保存最新截图。模型截图使用无密钥、无网络的 app-private catalog fixture。Android／Desktop 合并对照位于 `/Users/nanzhufeng/.codex/visualizations/2026/09/01/01a05b3f-bf39-7580-97c0-0c60854681f3/`。
- **门禁：** lint、typecheck、protocol golden、Node `135/135`、静态 build、Rust `163/163`、`cargo check` 全部通过，零失败、零跳过／ignored；release bundle 和严格 codesign 通过。详见 [同步交付](../ANDROID_DESKTOP_VISUAL_SYNC_20260901.md) 与仓库根 [design-qa](../../design-qa.md)。
- **边界：** 当前 bundle 仍为 ad-hoc 开发签名，不是 Developer ID／公证发行包。未调用真实 Provider／账单、Google／Supabase 或真实通知点击；未运行任何 `connected*AndroidTest`，未连接、安装或操作 OPPO。

## 2026-09-01：Desktop 未读水位与退出后后台运行时已落地（外部验收仍待完成）

- **未读 owner：** schema 33 按 workspace／conversation 保存 read／completed watermark 与手动未读时间；完成消息幂等，同题新 Attempt 独立推进。左侧只显示一个主题色圆点，手动未读在 pinned／recent 分区内排序；真正打开才清除，后台完成不抢当前路由。该状态为设备本地元数据，备份恢复保留当前设备值且不进入账号同步。
- **退出与后台：** schema 34 保存 content-free 后台状态。普通退出先隐藏窗口并排空在途生成，30 分钟发取消、31 分钟兜底退出；强制结束仍按 UNKNOWN 恢复、不自动重发。ACTIVE／RUNNING 提醒或历史整理开启时，固定 LaunchAgent 每 60 秒唤醒 headless cycle；根锁阻止与前台实例并发。全量隐私删除或最后消费者关闭后立即撤销 agent。
- **系统级证据：** `launchd-acceptance` feature 在唯一 `/tmp` home／root／script／label 下完成 bootstrap、真实执行和 bootout，`1/1` 通过，结束后无 service／plist 残留。正式 Desktop 数据、附件、Key、账号和误建会话均未触碰。
- **最新 Release 产物：** 当前源码已重建 [南枫 AI Desktop.app](../../desktop/src-tauri/target/release/bundle/macos/%E5%8D%97%E6%9E%AB%20AI%20Desktop.app)。版本／BundleVersion `0.6.0-p6d-dev`，Bundle ID `com.nanzhufeng.ai.desktop`，arm64；主程序 `30,629,280` bytes，SHA-256 `000b2d14b35446e6793e000ab890f915833dbd06063734964f6631966feb2d22`。严格 codesign 通过，但为 ad-hoc、`TeamIdentifier=not set`，`spctl` 拒绝，不能当作 Developer ID／公证包。bundle 内无平行 helper／plist；schema 34、未读／后台表、进程锁、退出排空与固定 headless 参数均直接进入主程序。
- **隔离启动烟测：** 唯一显示名“南枫 AI 未读后台验收 8243”、唯一 Bundle ID `com.nanzhufeng.ai.desktop.compareacceptance.8243.mti5l5f2`、独立 `/tmp` HOME／数据根实际启动。回读 schema 34、两张新表、`unread_indicators=1`、后台 `NOT_REQUIRED` 和 `integrity_check=ok`；正常退出后再运行 bundle 主程序的 `--nanfeng-background-cycle-v1` 返回 0。最终无进程、launchd service、plist 或临时目录残留。
- **门禁：** Node `128/128`、Rust 默认 `161/161`，均 0 failed、0 skipped／ignored；lint、typecheck、protocol、静态 build、`cargo check` 与 scoped `git diff --check` 通过。全库 `cargo fmt --check` 仍受既有在途 Rust 格式差异影响，本轮未扩大格式化范围。
- **总目标裁决：** 本地 Android 对齐 owner 已基本齐全，但有 Team ID 的通知热／冷真点、真实 Provider／账单、真实 Google／Supabase 双设备、真实 Qwen 长音频／SenseVoice 质量及 Compare 原生 Stop 点击仍待分层验收。详见 [最终审计](../ANDROID_DESKTOP_FINAL_COMPLETION_AUDIT_20260901.md)、[本增量记录](../DESKTOP_UNREAD_BACKGROUND_RUNTIME_ACCEPTANCE_20260901.md) 与 [提醒专项矩阵](../DESKTOP_REMINDERS_MONITOR_NOTIFICATIONS_ACCEPTANCE_MATRIX_20260901.md)。
- **边界：** 未运行任何 `connected*AndroidTest`，未连接、安装或操作 OPPO；未调用真实 Provider／Google／Supabase。

## 2026-09-01：Desktop macOS 原生通知点击桥已实现、受信任签名真点 P0 仍待验（整体目标仍未完成）

- **唯一 owner：** Rust 现统一负责通知发送与点击。受信任签名包优先 `UNUserNotificationCenterDelegate`，注册失败才使用 `NSUserNotificationCenterDelegate` 开发降级；JS plugin notify/onAction 路径已移除。发送只从 SQLite pending 投影重读，点击 payload 只含 `clickId/route/planId/workspaceId/conversationId`，导航前再查 SQLite，错 route、跨 workspace 或已删 plan 均 no-op。
- **队列与启动：** delegate 在 Tauri setup 早期安装，热启动通过固定 event 消费，冷启动通过有界队列 drain；handled `clickId` 去重，窗口只在目标二次校验后 show/unminimize/focus。桥初始化或前端监听失败不中断工作区启动。
- **隔离原生证据：** 唯一 Bundle ID `com.nanzhufeng.ai.desktop.nativeclick.build1788231149`、根 `/tmp/nanfeng-ai-desktop-ordinary-chat-acceptance.gCOVYf` 和 localhost mock 完成授权提示、多条一次性计划、多个 `reminder-run-*` 系统投递、完整退出与同根冷启动。legacy 通知在 macOS 26 未持久进入通知中心，因此不能冒充冷启动真点。
- **尚未闭环：** 当前开发 bundle 是 ad-hoc，`TeamIdentifier=not set`，`usernoted` 拒绝现代 UN client 注册。本轮未使用真实签名凭据，所以热／冷真实点按仍是 P0；未开始 unread read marker。唯一下一步是使用有效 Team ID 的独立验收包完成到期投递、热点、冷点、多通知／重复／已删除 no-op。
- **门禁与产物：** Node `124/124`，Rust `157/157`，0 failed、0 skipped/ignored；lint、typecheck、protocol、build、`cargo check`、bundle、严格 codesign、`git diff --check` 通过。开发 App 可执行文件 SHA-256 `c7acd23e5cf657de9d9ebe7ed1857102477b1532d586e4d2fe5387a7f1551883`。详见 [验收矩阵](../DESKTOP_REMINDERS_MONITOR_NOTIFICATIONS_ACCEPTANCE_MATRIX_20260901.md) 与 [续作交接 C](../HANDOFF_DESKTOP_REMINDERS_MONITOR_NOTIFICATIONS_20260901_C.md)。
- **边界：** 未读取正式 Desktop 数据、用户附件、Key 或真实账号；未调用真实 Provider／Google／Supabase；未操作 OPPO；未运行任何 `connected*AndroidTest`。

## 2026-09-01：Desktop 提醒发送链已验、macOS 原生点击桥 P0 待续（历史，已被上节更新）

- Browser 宽／窄屏、原生 localhost 一次性提醒、暂停／恢复、跨重启持久化、通知开关、`SUCCEEDED + SENT` 回执、Usage Ledger、最终 release bundle／严格 codesign 与全量零失败门禁已完成；Node `124/124`、Rust `154/154`。
- 已定位并修复 `notification.onAction` 注册失败导致整个 Desktop 启动读取中断；最终全新隔离 Bundle 已完整启动。新增通知 payload 白名单路由与构建清单回归。
- **提醒增量仍未完成：** Tauri notification 2.4.0 的 macOS desktop owner 未注册动作监听，当前系统通知真实发送成功，但点击无法可靠回传目标。下一任务必须先实现项目内 macOS 原生通知点击桥并真点打开精确 plan，再补原生 FAILED／UNKNOWN／运行中重启证据；未读 read marker 不得提前开始。
- 固定续作入口见 [Desktop 提醒／计划监控／系统通知续作交接 C](../HANDOFF_DESKTOP_REMINDERS_MONITOR_NOTIFICATIONS_20260901_C.md)。

## 2026-09-01：Desktop 提醒／计划监控／系统通知实现完成、原生验收待续（整体目标仍未完成）

- schema 32、审阅型草案、持久化计划、DST／错过策略、scheduler、UNKNOWN／显式重试、通知安全回执、设置 UI、Usage Ledger、备份与账号同步代码已实现；Node `123/123`、Rust `154/154`，均零失败且无跳过／ignored，lint、typecheck、静态 build、`cargo check` 通过。
- 当前**不能写本增量完成**：Browser 宽／窄视觉、唯一 Bundle ID＋唯一 `/tmp` 的 Tauri localhost 原生链、系统通知授权／触发／点击路由、跨重启实跑、最终 bundle/codesign 和验收矩阵仍待完成。
- 固定续作入口见 [Desktop 提醒／计划监控／系统通知续作交接 B](../HANDOFF_DESKTOP_REMINDERS_MONITOR_NOTIFICATIONS_20260901_B.md)。完成上述验收后才进入 unread read marker；Android→Desktop 总目标继续保持进行中。

## 2026-09-01：Desktop 历史资料库自动整理与 interests 保真（整体目标仍未完成）

- **隐藏字段保真：** Android 页面已隐藏但 domain 仍保留的 `interests` 现由 Desktop 独立 portable SQLite 表保存；普通聊天在 Memory 开启时作为“关注方向”消费，回答审计只记标题不复制正文。P6-K profile `publicBio` 只补空值；本机备份和用户显式选中对话的 Google 加密同步均保真。Desktop UI 未新增可见输入框。
- **真实整理 owner：** schema 31 新增全局 12 小时 schedule、candidate 与 checkpoint。只读取一个会话的当前成功文本分支；附件、工具、兄弟分支、FAILED／CANCELLED／UNKNOWN／PARTIAL 全部排除。请求前提交 reservation；响应严格 JSON 且高置信才成为 `PENDING_REVIEW`。中断为 `UNKNOWN`，不自动重发；显式重试新 Attempt、复用 checkpoint 并重新校验 source hash。
- **UI 与 Knowledge：** 设置“历史资料库”能力已启用；工作区“资料候选”与既有 Knowledge 分区。候选显示来源、实际模型、Token／费用、checkpoint 和失败原因，可编辑、接受、驳回、重试或删除。只有确认才调用现有 domain mutation owner；删除已接受的候选记录不连带删除 Knowledge。
- **视觉修复：** Browser `390×844` 发现设置一级／二级并排导致详情竖排裁切，现改为窄屏“菜单 → 单页详情 → 返回菜单”，`1440×960` 仍保持双栏；对应 Node 合同已补。
- **验证：** Node `120/120`、Rust `148/148`，均为 0 failed、0 skipped／ignored；lint、typecheck、protocol golden、静态 build、`cargo check`、macOS release bundle 和严格 codesign 已通过。专项状态机含当前分支、12 小时幂等、UNKNOWN、显式重试、低置信拒绝、接受与删除回执。
- **localhost 原生闭环：** 唯一 Bundle ID `com.nanzhufeng.ai.desktop.compareacceptance.81079.mthyir5e`、独立根 `/tmp/nanfeng-ai-desktop-ordinary-chat-acceptance.OJOmyC` 生成 4 消息候选；回读 `OPENROUTER / fixture-history-actual`、输入 41／输出 67、费用未知。编辑标题并确认后，候选为 `ACCEPTED`，独立 Knowledge 为 `ACTIVE r1`。signed executable SHA-256 `76e3a00a83d7e1ae2cb8233a41ca276d9189d7f1888a831ffe410edc5f3794a8`。
- **边界与下一优先：** 只用 localhost mock；未读取正式 Desktop 数据、用户附件、Key 或真实账号，未调用真实 Provider／Google／Supabase，未运行 `connected*AndroidTest`，未操作 OPPO。当前合同见 [Desktop 历史资料库与隐藏 interests 当前合同](../DESKTOP_HISTORY_KNOWLEDGE_CONTRACT.md)，证据见 [专项验收矩阵](../DESKTOP_HISTORY_KNOWLEDGE_ACCEPTANCE_MATRIX_20260901.md)。Android → Desktop 总目标继续进行中，下一项固定为提醒草案／计划监控／系统通知，其后是未读 read marker。

## 2026-09-01：Desktop Compare 真实执行链（整体目标仍未完成）

- **真实 owner：** Desktop Composer 只保留一个“对比”入口；旧 JS fail-closed owner 与 mock-only Rust owner 已删除。Tauri 新增 `submit/retry branch/cancel/read desktop compare` 命令，schema 30 把一次 USER、两条 sibling Assistant、两条普通聊天 Attempt 和一个 content-free execution envelope 原子关联。ChatGPT 固定 OpenRouter `GPT_5_6_TERRA`，Claude 固定 `CLAUDE_SONNET_5`；缺设置或凭据时失败关闭，不 fallback。
- **复用与状态：** 两分支复用普通聊天的当前路径、语气、自定义指令、Memory／Knowledge、网页检索、附件投影、实际模型、token、费用、诊断与 Usage Ledger；每支拥有独立 Attempt／idempotency／正文／错误／UNKNOWN／显式重试。同文再次点击创建新 execution。进程中断恢复 UNKNOWN，不自动重发；任一 Stop 指向同一 execution 的两条取消信号并保留已生成内容。
- **UI 与原生修复：** 原生验收发现并修复输入后 Compare 未即时启用；又发现打包环境事件桥不产生实时投影，现由普通聊天与 Compare 共用的只读 workspace 轮询显示 PARTIAL／Stop，且不会在用户切换会话后抢回页面。Browser 1440×900／780×700 只有一个入口、无横向溢出、0 error／warn。
- **localhost 原生证据：** 唯一 Bundle ID＋随机 `/tmp` 根完成双成功（每支实际模型与 `$0.000012`／`$0.000015`）、单失败＋只重试 Claude、双失败、双 UNKNOWN／保留部分文本，以及实时双 PARTIAL／双 Stop 实看。Computer Use 因动态重渲染导致两个同名 Stop 元素持续失效，未取得原生点击后的双 `CANCELLED` 回读；停止终态只由 transport cancellation 与 SQLite 测试覆盖，不能写成原生点击闭环。
- **最终回归／产物：** Node `117/117`、Rust `139/139`，均 0 failed、0 skipped／ignored；Compare Rust 定向 `3/3`（含双分支 `CANCELLED` 与部分文本保留）。lint、typecheck、protocol golden、静态 build、`cargo check`、macOS release bundle 与严格 codesign 通过。最终隔离副本 Bundle ID `com.nanzhufeng.ai.desktop.compareacceptance.74278.mthwrm6q`，根 `/tmp/nanfeng-ai-desktop-ordinary-chat-acceptance.mDRURW`，signed executable SHA-256 `74cd8fa8435f7cc2b7922471139827c0903671c990d6990e2cf019ece3c8b7a4`；该最终副本只完成封装／验签。原生场景实跑使用前一份已含核心链与 PARTIAL 轮询修复的隔离 Bundle；最终副本只额外加入“轮询不抢回已切换会话”的保护。
- **合同与矩阵：** 当前长期合同见 [Desktop Compare 真实执行合同](../DESKTOP_COMPARE_EXECUTION_CONTRACT.md)，逐项状态和外部边界见 [Desktop Compare 专项验收矩阵](../DESKTOP_COMPARE_EXECUTION_ACCEPTANCE_MATRIX_20260901.md)。真实 OpenRouter／Key／账单未调用；正式 Desktop 数据根、用户附件与 OPPO 未触碰，未运行任何 `connected*AndroidTest`。
- **下一优先：** Compare 的真实本地核心链不再是 P0 缺口。总目标继续保持进行中：下一项为历史资料库低频自动整理＋`interests` 保真，其后是提醒／计划监控／系统通知／未读 read marker；真实 Provider Compare 与稳定原生 Stop 点击另按分层授权补验。

## 2026-09-01：Android → Desktop 最终完成审计续作交接（历史快照，已被上节更新）

- **本轮完成：** Desktop 普通聊天已真实消费基础语气、自定义指令、昵称／职业、相关 Memory 与相关 Knowledge；增加 Provider 无关的首轮昵称兜底、回答级 content-free 上下文审计与回答底部入口。网页检索按 Android live adapter 接入 OpenRouter、智谱、Qwen 两条路径和 DeepSeek Responses；新增 route 审计、严格终态、来源去重与近 7 天 FAILED／UNKNOWN 安全诊断。
- **必须保持未完成：** Compare 仍只有 fail-closed JS owner 和注入 mock 的 Rust receipt，现有测试明确断言“无 Tauri 注册／无真实 HTTP client”；历史资料库没有低频自动整理完整 owner；计划监控通知、提醒建议和未读水位没有消费者。Android 当前个性化页面虽不再显示“更多信息”，但 live domain／偏好仓库／普通请求仍保留 `interests`（“关注方向”）兼容字段，Desktop 尚未保真保存或消费。能力均继续禁用，不得因为全量测试通过而写成全功能完成。完整逐域矩阵见 [Android → Desktop 最终完成审计](../ANDROID_DESKTOP_FINAL_COMPLETION_AUDIT_20260901.md)。
- **验证：** Node `120/120`、Rust `143/143`，均 0 failed、0 skipped／ignored；lint、typecheck、protocol golden、静态 build、macOS Release bundle 和严格 codesign 通过。Browser 实看 `1280×800`、`820×720`、`780×720`，控制台 0 error／warn。
- **原生隔离：** 唯一 Bundle ID `com.nanzhufeng.ai.desktop.p6v2pickeracceptance.66701.mthusejf`，独立根 `/tmp/nanfeng-ai-p6-v2-picker-acceptance.final-audit-v2.ZK44SK`，schema 29、0 工作区、0 普通聊天 Attempt。原生实看首页、个性化、模型与联网、运行诊断和提醒禁用态；发现并修复“无 owner 但兼容值为真时禁用提醒开关仍呈橙色”的视觉误导，复验三个开关均为灰色关闭。
- **数据边界：** 正式 Desktop 数据根、附件、Key 和此前误建的 `conversation-chat-create-mthqap1s-77j3l2da` 均未读取、修改或删除。未调用真实 Provider／Google／Supabase，未运行任何 `connected*AndroidTest`，未连接、安装或操作 OPPO。
- **下一任务第一优先：** 先实现 Compare 的真实 Tauri／HTTP／SQLite／UI／费用／停止／错误／UNKNOWN／重启链；随后实现历史资料库自动整理并补齐 `interests` 导入／持久化／请求保真，再实现提醒草案、计划监控、系统通知与未读 read marker。总目标继续保持进行中。

## 2026-09-01：Desktop Google 账号、恢复码与南枫云端到端加密同步

- **本轮结论：** Desktop 客户端已补齐 Google OAuth PKCE、Supabase 应用会话、App 专属 Keychain、恢复码一次显示／轮换／中断续办、账号切换／退出保留本地、用户显式选中的纯文本对话 E2EE 同步、冲突防覆盖、未知提交不重发、新设备恢复到新工作区、定期同步、安全通知与诊断。详细 owner、流程、测试与边界见 [Desktop Google 账号与南枫云加密同步交接](../DESKTOP_ACCOUNT_SYNC_HANDOFF_20260901.md)。
- **数据与失败语义：** schema 升至 v28；云端只接收密文 envelope，SQLite 不保存 Token、Key、恢复码、正文、路径或 URI。附件／工具结果／草稿／未完成或已删除内容在加密前拒绝。每次提交必须回读；revision 冲突或结果未知均停止自动进行。
- **验证：** Node `119/119`、Rust `139/139`，全部 `0 failed`；lint、typecheck、静态 build、`cargo check`、macOS release bundle 与严格 codesign 均通过。Browser 实看宽屏与 `720×900` 窄屏，会话右键“设置加密同步”实点进入账号页。开发包版本 `0.6.0-p6d-dev`、Bundle ID `com.nanzhufeng.ai.desktop`；签名无 Team ID，不是公证发行包。
- **边界：** 只用 localhost Mock，未访问真实 Google／Supabase／真实账号或 Keychain；生产 RPC 仍需真实部署合同验收。因当前开发包与正式窗口共用 Bundle ID，本轮没有启动原生 GUI，避免命中正式数据。未运行 `connected*AndroidTest`，未连接或操作 OPPO。

## 2026-09-01：Desktop 设置全量视觉、SQLite 持久化与 Release 验收

- **范围与代码修复：** 本轮只验收 Desktop 设置，不新增功能。浏览器逐页实看 10 个一级页、10 个二／三级页与 3 个外观选择器后，修复了 Tauri 最小窗口 `780×560` 下 `.app-shell.chat-first` 的 `620px` 最小高度导致整窗滚动、标题可被卷走的问题；设置模式现在由内部滚动 owner 承担。双实例 revision 验收又发现 Android 风格设置壳遗漏 `error` 投影，底层虽拒绝 stale 写入但页面不显示原因；现已改为优先显示错误并增加 Node 断言。
- **视觉验收：** 应用内 Browser 在 `1280×800`、`820×900` 与 Tauri 最小窗口 `780×560` 完成全页实看；灰底画布、纯白卡片、字段标题上置、无 floating label、同轮廓 hover／pressed／focus、窄窗内部滚动与无横向溢出均通过。控制台无 app error／warn。关键证据位于 `/tmp/nanfeng-ai-settings-qa-uRmbHs/`，包括 `wide-contact-sheet.jpg`、`narrow-contact-sheet.jpg`、`40-min-development-fixed.png` 与 `41-native-revision-conflict.png`。
- **隔离原生持久化：** 仅使用唯一验收 Bundle ID 与 `/tmp/nanfeng-ai-p6-v2-picker-acceptance.*` 数据根，未读取正式 Desktop 数据。受控 v1 交换包通过严格预检导入独立工作区；会话经原生右键菜单收藏，SQLite `desktop_conversation_favorites` 写入后完整退出、重启仍显示“已收藏”。外观设置写入 `desktop_app_settings` 并跨重启恢复。两个最新 Release 隔离实例共同读取 revision `1` 后，第一实例写入 `green / revision 2`，第二实例 stale 写入被拒绝并明确显示“设置已在其他窗口更新；请刷新后重试”，数据库保持第一实例真值。
- **旧偏好一次迁移：** 用同一唯一验收 Bundle ID 的临时 WebKit 夹具预置旧 `nanfeng-ai.desktop.appearance.v1` 与 `nanfeng-ai.desktop.product-settings.v1`，再以全新数据根首次启动 Release Tauri。SQLite 回读为 `dark / large / pink`、昵称“迁移验收”、`revision 1`；两条旧 localStorage 随后均回读为 `null`。第二次完整启动后值不变且 revision 仍为 `1`，证明迁移只执行一次。该夹具和数据根只用于隔离验收，不进入产品代码或正式数据。
- **自动验证与 Release：** `npm run lint`、`npm run typecheck`、Node 全量 `115/115`、protocol golden、`cargo check`、Rust 全量 `124/124`、静态 build 与 macOS Release bundle 均通过，均为 0 failure／0 skipped（Rust 0 ignored）。正式开发包为 [南枫 AI Desktop.app](../../desktop/src-tauri/target/release/bundle/macos/%E5%8D%97%E6%9E%AB%20AI%20Desktop.app)，版本 `0.6.0-p6d-dev`、Bundle ID `com.nanzhufeng.ai.desktop`、arm64、主可执行文件 SHA-256 `dcd9aff25c6e8c87351fdef8462e74a2beb5cd3907c7a7b3294c42d134ff8528`。严格 codesign 校验通过，但签名是 ad-hoc、无 Team ID，Gatekeeper `spctl` 拒绝；因此这是可本机开发验收包，不是 Developer ID／公证发行包。
- **边界与剩余工作：** 未调用 Google／Supabase、Provider、系统通知或更新服务，未读取凭据或真实账号；无 Desktop 消费者的个性化、提醒和网页搜索控件继续禁用并说明原因。未运行任何 `connected*AndroidTest`，未连接、安装或操作 OPPO。Desktop 设置本增量已完成；整个 Android → Desktop 同步仍需继续审计真实聊天／流式状态、附件发送与回读、Provider／账号同步及诊断等非设置模块，不能把本轮完成误写为整体完成。

## 2026-09-01：Desktop 南枫转写（语音）Android live source 对齐

- **能力边界：** 本增量同步的是 `NanfengTranscriber-Android` 当前存在的音频／视频 → 文字工作流；`Nanfeng_AI` 原有 GLM-OCR 图片／PDF → Markdown 继续独立。Desktop 支持与 Android 相同的 17 种音视频扩展名、自动／中文／英文、Qwen3-ASR、任务队列／取消／显式重试／启动恢复、原文件预览、全文／切片时间轴、复制、继续对话，以及 TXT／MD／SRT／DOCX 原子导出。Android 当前没有录音和暂停，Desktop 没有伪造这两个动作。完整矩阵见 [Desktop 语音转写 Android 对齐矩阵](../DESKTOP_SPEECH_TRANSCRIPTION_ANDROID_PARITY_MATRIX.md)。
- **真实 owner：** SQLite schema 升至 24，新增设置、任务与分段 owner；源文件先流式复制并计算 SHA-256，再进入共享附件引用、搜索、备份、隐私统计与本地删除链。没有旧工作区的新用户会自动使用不出现在普通工作区列表中的“南枫转写（本地）”私有空间，不再被旧工作区导入前置条件阻塞。Qwen 固定使用现有 QWEN Key owner、DashScope OpenAI-compatible 端点与 `qwen3-asr-flash`，由 `ffprobe` 校验、`ffmpeg` 生成 16 kHz 单声道 PCM WAV，并按 5 分钟切片持久化进度、请求数、计费音频时长与人民币微单位估算。SenseVoice Small INT8 只显示“验证中／保留实验”，不可选、不可静默回退。
- **界面与联动：** 左侧新增“南枫转写”独立工作页，灰底画布与亮白卡片内集中模型／语言／默认导出、任务列表、状态、进度、费用、错误、源文件和结果动作；设置 → 开发与诊断同步提供“功能审阅”，不在聊天首页或 Composer 增加常驻按钮。完成结果作为共享 Markdown 附件进入新对话草稿，搜索结果可回到稳定任务 ID。
- **自动验证：** Desktop 前端 `114/114`、Rust 全量 `120/120`，均为 `0 failed / 0 ignored`；新增无工作区导入／取消／重试定向测试通过。`npm run lint`、静态 build、release `.app` bundle、ad-hoc codesign 与严格签名校验通过。正式开发包为 [南枫 AI Desktop.app](../../desktop/src-tauri/target/release/bundle/macos/%E5%8D%97%E6%9E%AB%20AI%20Desktop.app)，构建时主可执行文件 SHA-256 为 `8f0fba19e172d6ba0ecf953173a334c0fcc1f008a39f42eb22d8c7535acb821b`。
- **隔离可视验收：** 应用内 Browser 在 `1280×800` 与 `480×900` 实看页面身份、空态、模型／语言／导出设置、SenseVoice 实验说明、功能审阅与单列响应式布局；无横向溢出、无 console error／warn，Web 预览不会读本机文件。另以唯一 Bundle ID `com.nanzhufeng.ai.desktop.p6v2pickeracceptance.37053.mthjv9kx` 和全新 `/tmp/nanfeng-ai-p6-v2-picker-acceptance.transcription.fTtym7` 启动 Release Tauri，实测在没有旧工作区时选择一段 `64,078` bytes／2 秒 WAV，私有资产 SHA-256 `7695bcad887367c33cf9f9bce6bf0d98b4fd1547d1b2f9b392c4d426ef7a33c1`、引用计数 `1`、任务 `QUEUED`，取消后为 `CANCELLED`，完整退出再启动仍可恢复同一任务。
- **仍待真实验收：** 本轮没有读取用户 Qwen Key、没有发出真实 Qwen 请求，所以 Provider 返回、长音频切片、无语音判定、真实 Token／账单与四种结果导出的原生交互尚未做真实账号闭环；SenseVoice 的模型资产、许可、速度／内存／温控和中英混合准确率仍未完成。未运行任何 `connected*AndroidTest`，未连接、安装或操作 OPPO。

## 2026-09-01：Desktop 完整全屏搜索纵向增量

- **界面与交互：** 侧栏搜索现进入独占全屏灰底画布，提供全部／正文／图片／视频／音频／文件六分类、时间／大小全局排序与还原、文件类型筛选、结果计数和月份分组。底部悬浮搜索框支持显式提交历史、精确／前缀／包含联想、关键词高亮、加载／空／错误态；返回会话后可回到原搜索位置。正文结果能按稳定消息 ID 定位；隐藏分支先由 Rust `switchLeaf` owner 以 revision 门禁切换，冲突时停留搜索页。
- **真实索引与附件：** SQLite schema 升至 23。派生索引覆盖未删除会话的标题、完整消息树与每个附件 occurrence，并保存 workspace／conversation／message／attachment 稳定 ID、真实 MIME、大小、消息时间、分支叶、归档状态与导入来源。附件卡复用本地图片／PDF／视频／音频／安全文本预览；其他已验 hash 的私有附件只在用户明确点击后交给 macOS。删除只移除所选消息中的一个附件引用；共享字节保留，最后引用移除后仍由既有 24 小时安全维护 owner 清理。
- **验证与缺口：** Desktop 前端 `112/112`、Rust `115/115`，均为 `0 failed / 0 ignored`；lint、typecheck、静态 build、`cargo check`、release `.app` bundle 与 codesign 校验通过。隔离浏览器实看宽屏和 820×720 窄屏的分类、文件类型、排序、历史与诚实 Web 错误态，控制台无 error／warn；过程中发现并修复静态 build 漏复制 `desktop-search-page.mjs`。另以唯一 Bundle ID、全新 `/tmp` 数据根启动真实 Tauri，实看空库首页、全屏搜索和文件分类。未读取正式 Desktop 用户数据，未调用真实 Provider／账号，未运行 `connected*AndroidTest`，未安装或操作 OPPO；真实大规模用户库的滚动性能与真实附件系统打开仍待后续专用数据验收。

## 2026-08-31：Desktop 本机备份／恢复纵向增量

- **当前基线与 owner：** 本轮只读取当前 checkout 中 Android live source、`AndroidLocalBackupRestoreManager`、`LocalBackupRestoreUi` 与 [P5-D 本机备份恢复合同](../P5D_LOCAL_BACKUP_RESTORE_DELIVERY_CONTRACT.md)；`f596b8b` 仅是历史快照，不再作为 Desktop 同步基线。Desktop 新增原生系统保存／打开选择器入口和 Rust `desktop_local_backup_v1` owner：导出用 `VACUUM INTO` 取得一致性 SQLite 快照，排除设备专属／派生状态，完成 ZIP 回读、entry 哈希及所选目标文件 SHA-256 回读后才成功。
- **恢复与数据边界：** 预检先复制到 App 私有 inbox，再检查格式、Schema、路径、重复项、符号链接、展开上限、压缩比、SQLite 完整性、表计数、敏感值和每个 entry 哈希。本地非空时必须显式选择“替换本地”或取消；恢复先创建数据库、费用账本和受控资产 checkpoint，再切换，失败自动回滚。Desktop 独立的 `usage-ledger-v1.sqlite3` 作为辅助 SQLite 一致性快照进入同一包；当前设备的 Provider 设置、模型目录／默认、同步元数据／意图和窗口布局不从备份覆盖。搜索／预览缓存不进包，恢复后的新进程先重建全部本地搜索索引，再清 checkpoint 并标记完成。
- **可见行为：** 设置 → 导入与导出保持 Android 分组和顺序；备份／恢复在原生壳可操作。预检显示格式、Schema、资产大小、表计数与冲突；强确认前“恢复并要求重启”禁用。切换成功后当前进程拒绝继续读写，页面固定提示完全退出并重新打开 App，且不会自动继续任何导入或生成任务。
- **验证等级：** Rust 定向 `3/3`、全量 `113/113`；Desktop 前端 `106/106`、`0 skipped`；`npm run lint`、`npm run typecheck`、`npm run build`、`cargo check` 和定向 `git diff --check` 通过。隔离浏览器投影已实看预检冲突、强确认门禁和重启态，控制台 `0` error。未为本增量重新打 macOS `.app`，未实际操作原生系统选择器或真实用户备份文件，未运行任何 `connected*AndroidTest`，未安装或操作 OPPO。
- **下一件事：** 继续按当前 Android live source 实现 Desktop 完整搜索 UI；在最终双端交付阶段再统一完成 `.app` bundle、隔离原生选择器与重启闭环实看。

## 2026-08-31：Android → Desktop 第一轮同步与完整差距审计

- **历史阶段结论：** 该阶段曾以旧快照整理第一轮差距；旧提交不再是当前基线。当前同步只以 Android live source 与当前合同为准。Desktop 已完成主题外观、收藏管理、会话内查找、原生 Markdown 导出、附件草稿预览复用、图片完整预览／缩放／平移、运行时版本信息及配套本地持久化与最小权限。完整逐项状态见 [Android → Desktop 双端同步审计](../ANDROID_DESKTOP_PARITY_AUDIT_20260831.md)。
- **不可误报的缺口：** 普通聊天真实 Provider、后台／多会话生成连续性、Google 账号与端到端加密同步、请求上下文投影、附件真实外发、用量费用、计划任务通知及语音转写仍未达到 Android 同级闭环；当前只能称“第一轮同步完成、整体同步进行中”。
- **验证：** Desktop 前端 `99/99`、Rust `99/99`；lint、typecheck、前端 build、`cargo check` 与 `cargo tauri build --debug --bundles app` 全部通过。浏览器完成默认宽屏、深色／大字体／绿色主题、查找、收藏和 820×720 窄窗实看；隔离 macOS 原生壳完成首页与“关于”页实看。未运行 `connected*AndroidTest`，未安装或操作 OPPO，未调用用户真实 Provider／Google 账号；调试 `.app` 不代表正式签名交付。
