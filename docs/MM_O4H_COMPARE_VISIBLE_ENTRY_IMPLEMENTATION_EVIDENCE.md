...
- Final strict-signed Desktop bundle was started against the P6-E isolated `/tmp` root. Its native Accessibility tree exposed all three Composer Compare affordances; clicking the empty-draft `对比 ChatGPT + Claude` entry visibly reported that the Desktop execution owner is unavailable and that no Key or content was sent. No confirmation Dialog appeared. The isolated root contained prior controlled acceptance conversations, so this readback intentionally reports no conversation, title, attachment, or body text.
