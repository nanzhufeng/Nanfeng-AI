...
- **普通聊天上下文：** 用户资料与自定义指令在个性化开启时参与每次普通调用；已开启的 Memory／资料库只在本机按当前问题检索相关项、完成预算裁剪后随本次请求发送。本文旧的“全量扫描／字符预算”风险描述已被当前 FTS 与 token-aware 实现取代；唯一正文为 [Android 当前运行时上下文合同](ANDROID_RUNTIME_CONTEXT_CURRENT_CONTRACT.md)。
