...
| 完整工作区交换（v2） / Android scope→mapper/package→SAF bridge；Desktop `p6_workspace_exchange_v2` private owner | 设置 → 数据与导入 → 完整工作区交换（v2）→ 显式范围 → 系统保存位置 | 设置 → 数据与导入 → 完整工作区交换（v2） | Android/Desktop 设置 → 功能审阅 | 候选；Android v2 导出 + Desktop v2 私有导入 | Android 尚未完成真实 DocumentsUI/readback 或 v2 import/archive；Desktop 未完成真实 native picker、跨端/Windows/发布验收 | Android 只在内存 strict preflight 后写一次 SAF 并回读 hash；失败请求删除未完成文档；Desktop 保持 strict preflight/v1 隔离 private archive/transaction/content-free receipt | Android JVM mapper/writer/scope 与 UI 合同；Desktop Rust/UI 合同；双端真实文件链待后续 |
