...
| 完整工作区交换（v2） / Desktop `p6_workspace_exchange_v2` private owner | 无；Android 仅功能审阅登记 | 设置 → 数据与导入 → 完整工作区交换（v2） | Android/Desktop 设置 → 功能审阅 | 候选；仅 Desktop v2 私有导入 | Android 无 mapper/package/UI chain；Desktop 未完成真实 native picker、跨端/Windows/发布验收 | 选择文件有限读入、strict preflight、v1 隔离 private archive/transaction、content-free receipt | Rust command/owner 合同、Desktop UI 合同；真实 picker fixture/readback 待后续 |
