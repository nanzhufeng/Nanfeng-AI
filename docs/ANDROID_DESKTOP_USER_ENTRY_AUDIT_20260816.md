...
- 当前登记项：ChatGPT / Claude ZIP 导入（建议保留设置入口、不加对话主页快捷键）；未关联媒体人工关联（建议仅在存在候选时于批次详情提供二级操作）；Desktop Compare 联网执行（待您判断，阶段 1–5 已完成 fail-closed owner、Security.framework 边界和未注册 mock-only adapter/安全 receipt；建议复用既有“对比”操作，不新增 Composer 常驻按钮；固定预设与状态仅由设置 → AI 模型服务承载）。
