...
| 完整工作区交换（v2） / Android scope→mapper/package→SAF bridge；Desktop `p6_workspace_exchange_v2` private owner | 设置 → 数据与导入 → 完整工作区交换（v2）→ 显式范围 → 系统保存位置 | 设置 → 数据与导入 → 完整工作区交换（v2） | Android/Desktop 设置 → 功能审阅 | 候选；Android v2 导出 + Desktop v2 私有导入 | Android 已在独立 AVD 经 DocumentsUI 导出/回读；Desktop native picker、跨端 committed reopen/re-export、Windows/发布仍待验收 | Android 建议 `.nfai-exchange`，`application/zip` DocumentsUI 可追加 `.zip`；Desktop 只容许精确 `.nfai-exchange` 或 `.nfai-exchange.zip`，二者均严格 preflight/v1 隔离/private archive/transaction/content-free receipt | Android 真实 DocumentsUI/readback + Node/Desktop strict reader；Desktop Rust/UI 合同，native picker 跨端真实链待后续 |
