# 南枫 AI 总控方案需求—证据完成审计（2026-08-16；2026-08-30 当前总控门更新）

## 2026-08-30 当前总控门：集成 checkpoint、统一 owner 与复查顺序

**本节是本文唯一的当前完成性与排程入口。** 下方 2026-08-28 及更早段落只保存当时的根因与证据，不得用旧 checkpoint、旧 Schema、旧测试数、旧 APK、旧设备状态或旧“下一任务”覆盖本节。功能细节仍由对应当前合同定义，本节只回答“现在以什么为准、哪些不再重做、还缺哪一层证据”。

### A. 当前基线与唯一事实源

- **代码与文档 checkpoint：** Android 集成代码冻结于 `b7e1c2f checkpoint(android): freeze integrated product baseline`，回归合同与增量文档冻结于 `1f7f356 test(docs): align final regression evidence`；当前 Room 为 Schema 63。后续若 HEAD 或工作树变化，必须先重新核验，不能继续套用这里的静态数字。
- **长期方向与复查路由：** 只读 [总控开发蓝图](MASTER_DEVELOPMENT_BLUEPRINT.md) 顶部 2026-08-30 总控门；蓝图内早期 P 阶段“当前状态”和“下一唯一入口”均已降为历史。
...
| 模型与 Provider | GLM-5.3 等模型经中央目录进入设置、手动／Auto、权限、Adapter、附件、调用归因和回归；标题与历史资料整理共用明确路由，Qwen 联网时限／终态按当前 Adapter 合同处理。 | 新模型不得只加入选择框；切换模型不得静默重投失败请求，后台路由不得写死旧 Provider。 |
...
| 完整 JVM | `1013 tests / 4 failures / 3 skipped`，相较首次 `12 failures` 新增回归为 0。4 项保留失败为 PDF renderer cache、统一 Dialog 遮罩、Dialog 内向边缘手势、设置画布静态合同。 | 不得宣称 JVM 全绿；也不得把 4 项历史失败误写成本轮模型、转写、搜索或存储回归。 |
| Lint／Release | `lintVitalRelease`、`assembleRelease` 通过。当前重建候选为 code 66 / `0.3.0-p10j`，SHA-256 `0dba16156f36e76a23fe52748ed0416c7126db1e3d6b207b896e24605a7adfe0`，v2/v3 正式证书通过。 | 可声称当前代码可生成正式签名候选；不可声称已安装、已发布或真机功能通过。 |
| OPPO | 当前已安装仍是前一轮保数据覆盖并回读的 SHA-256 `4f4344e7f13764e6f1e32e9eeba981fd30174995a42a9dccaf330bda8efdf464`。重建候选未再次覆盖。 | 不得拿新 APK 的代码能力描述当前设备；后续覆盖必须重新获得授权并重复身份、签名和数据指纹门。 |
| 真实服务 | 本次最终回归未使用用户 Key 发起 Qwen、GLM、DeepSeek 或其他真实 Provider 请求。 | 真实耗时、Token、费用、搜索终态、账号权限和服务端行为均仍待独立验证。 |

### D. 当前剩余任务与安全顺序

1. **历史 4 项回归债务：** 逐项以现行产品合同判断代码缺陷或静态断言过时；保持问题隔离，不在其他 UI／模型任务中顺手改动。
2. **OPPO 覆盖与目标视口回读（需再次明确授权）：** 只使用同包名、同证书、非 Debug 的当前稳定 Release 保数据覆盖；安装后分别回读 APK、数据指纹、搜索完整性、实际存储体积、南枫转写预览、费用排版、待看排序／圆点和暗色入口。
3. **真实 Provider 验收（需用户合法配置和明确发送）：** 分模型记录首字／终态耗时、reasoning／output Token、联网状态、工具终态、实际账单与失败恢复；不自动重试或换模型掩盖超时。
4. **Desktop 差异审计：** 只有用户要求跨端同步时，才按入口审计逐项确认 Android 新增能力在 Desktop 的去留、实现与证据，不把 Android checkpoint 外推到 Desktop。
5. **后续新增模型固定门：** 必须同时检查中央目录、Provider／凭据、手动与 Auto 顺序、附件材料桥、联网／推理参数、调用账本、费用诚实边界、设置、错误隔离和完整回归；任一缺失都不得写成“完整接入”。
...
## 2026-08-28 历史总控门：checkpoint、导入链、验证与冲突裁决（已被 2026-08-30 当前门取代）

**本节仅保留 2026-08-28 当时的稳定 checkpoint 与附件恢复证据。** 其中 `c1c9ae0`、Schema 56、819 项 JVM、旧 APK／OPPO 和“当前剩余任务”均已被上方 2026-08-30 总控门取代；只可用于追溯根因，不得继续作为排程入口。
...
本节只保留 2026-08-27 当时的合同路由和验证分层证据，不再是排程入口。其中 `796 / 60`、旧 Release 状态和“附件待验收”等描述先被 2026-08-28 历史门取代，现统一服从上方 2026-08-30 当前总控门；三份当前合同的路由依然有效。
