# 南枫 AI 总控方案需求—证据完成审计（2026-08-16；2026-09-03 当前总控门更新）
...
- 旧 `2026-08-31 当前总控门`、Schema／测试数／APK／OPPO 和 Desktop 完成声明均降为历史证据；不得覆盖当前交接和最终独立审计。

## 2026-08-31 历史总控门：模型、媒体与生成连续性 checkpoint

**历史记录。** 本节保存 2026-08-31 当时的根因与证据，不得用其中的 checkpoint、Schema、测试数、APK、设备状态或“下一任务”覆盖 2026-09-03 当前门。

### A. 当前基线与唯一事实源

- **代码与文档 checkpoint：** Android 当前 checkpoint 为 `d6db5bf checkpoint(android): stabilize model and media interactions`；当前 Room 为 Schema 63。它冻结模型退役失败关闭、生成期间导航连续性、草稿附件本地预览和原图查看几何合同。后续若 HEAD 或工作树变化，必须先重新核验，不能继续套用这里的静态数字。
- **长期方向与复查路由：** 只读 [总控开发蓝图](MASTER_DEVELOPMENT_BLUEPRINT.md) 顶部 2026-08-31 总控门；蓝图内 2026-08-30 及更早的 P 阶段“当前状态”和“下一唯一入口”均已降为历史。
- **实现、回归、Release 与设备：** 只读 [当前交接](CURRENT_HANDOFF.md) 顶部。APK 候选、OPPO 已安装包和真实 Provider 是三个独立事实，不得互相冒充。
- **Android 现行行为：** 会话／搜索／文件／主题读取 [Android 当前会话界面合同](ANDROID_CONVERSATION_UI_CURRENT_CONTRACT.md)，设置读取 [Android 当前设置界面合同](ANDROID_SETTINGS_UI_CURRENT_CONTRACT.md)，普通聊天上下文读取 [Android 当前运行时上下文合同](ANDROID_RUNTIME_CONTEXT_CURRENT_CONTRACT.md)。
- **专项 owner：** 模型与 Auto 读取 [P6-G 模型选择合同](P6G_MODEL_SELECTION_AUTO_ROUTER_CONTRACT.md)，南枫转写读取 [GLM-OCR 当前合同](GLM_OCR_DOCUMENT_MARKDOWN_CONTRACT.md)，ChatGPT／Claude ZIP 读取 [P6-K 导入合同](P6K_CHATGPT_CLAUDE_ZIP_IMPORT_ADOPTION_CONTRACT.md)，费用读取 [AI 用量与费用合同](AI_USAGE_COST_AND_BALANCE_LEDGER_CONTRACT.md)。长期 owner 变更才进入 [决策日志](decision-log.md)。
- **跨端边界：** 本次 checkpoint 的新增收口主体是 Android；Desktop 继续保持既有已验证基线。除非 Desktop 源码和独立证据明确完成，不得因 Android 同名入口而写成双端同步。

### B. 当前已收口的集成范围（后续复查忽略重做）

| 范围 | 当前结论 | 禁止重新割裂 |
| --- | --- | --- |
| 会话、搜索与文件 | 普通会话、ChatGPT／Claude 导入内容、南枫转写和普通附件共用搜索时间线、文件卡、预览／打开、复制／下载／分享及返回层级；正文索引覆盖导入消息和确定分支。 | 不为南枫转写另建搜索分组、专用第三方查看器或不同点击语义；不把已导入正文排除在“正文”搜索外。 |
| 私有附件与导入 | MD／文本、PDF、图片、视频等通过能力匹配、本机提取或材料桥进入模型；ZIP 恢复、引用计数、原包清理与最后引用回收继续由既有 owner 管理。 | 不把 Provider 原生多模态误写成所有格式都原生支持；不绕开完整性、归属、重复计费和最后引用门。 |
| 南枫转写 | GLM-OCR 已纳入模型设置、统一调用账本、费用、搜索、文件生命周期和 App 内预览；列表、详情和加入新对话不再是体系外孤岛。 | 不复制第二套文件、账本、统计、预览或搜索 owner；不恢复额外外发勾选。 |
| 模型与 Provider | 模型经中央目录进入设置、手动／Auto、权限、Adapter、附件、调用归因和回归；退役模型只保留历史归因，旧选择在读取凭据和外发前 `MODEL_NOT_FOUND` 失败关闭。标题与历史资料整理共用明确路由，Qwen 联网时限／终态按当前 Adapter 合同处理。 | 新模型不得只加入选择框；切换模型不得静默重投或替换失败请求，后台路由不得写死旧 Provider。 |
| 生成连续性与媒体查看 | 生成任务由会话 owner 持有，切换对话、设置、搜索或应用界面只更换 UI 投影；草稿附件复用已发送附件的本地预览投影，原图按真实几何溢出缩放／平移。 | 不把任务绑到页面生命周期，不把草稿降级为通用图标，不以固定缩放阈值或位置限制图片查看。 |
| 存储与统计 | 本机数据按活动 owner、唯一受管文件和当前物理字节统计；无 owner 残留单列，显式清理；原 ZIP 只按当前保留物理字节计一次。 | 不再用导入清单、历史目录行或已删除引用冒充实际占用；南枫转写文件不得漏计或重复计。 |
| 费用与会话状态 | 会话、标题整理、历史资料、南枫转写统一投影既有调用账本；金额保留实际／估算边界。会话“待看”在原分组内置顶，左侧主题色圆点只在真正打开时清除。 | 不建立第二份费用账本，不把估算写成实扣；不把长按、菜单或其他行点击误判为已查看。 |

### C. 当前验证、产物与设备边界

| 验证层 | 当前证据 | 结论边界 |
| --- | --- | --- |
| 本轮定向回归 | 首次全量暴露的 8 项旧静态／Room 合同已按当前行为更新，精确 `8/8` 通过。 | 可确认本轮合同对齐；不能替代全量、真机或真实 Provider。 |
| 完整 JVM | `1042 tests / 0 failures / 3 skipped`。此前 5 项失败均为静态断言与已演进源码的锚点漂移，已在不改运行逻辑的前提下按当前行为校正。 | 可声称 JVM 全绿；3 项 skip 仍不得写成已执行，且完整 JVM 不替代真机或真实 Provider。 |
| Lint／Release | `lintVitalRelease`、`assembleRelease` 通过。当前正式候选为 code 66 / `0.3.0-p10j`，SHA-256 `8279335eef23b7aff39fbf08ff367e2a7d3ec8b0325f04f5d0e3062aabe95593`，v2/v3 正式证书通过。 | 可声称当前代码可生成正式签名候选；不可声称已发布或真机交互已经人工验收。 |
| OPPO | 与当前候选相同 SHA-256 的正式包已完成同签名保数据覆盖并逐字节回读，首装时间与 CE／DE 数据 inode 不变。 | 仅证明安装与数据保留；不替代图片手势、生成连续性或真实模型回复的人工回读。 |
| 真实服务 | 本次最终回归未使用用户 Key 发起 Qwen、GLM、DeepSeek 或其他真实 Provider 请求。 | 真实耗时、Token、费用、搜索终态、账号权限和服务端行为均仍待独立验证。 |

### D. 当前剩余任务与安全顺序

1. **OPPO 人工交互回读：** 当前正式包已保数据覆盖；仅在南烛枫实际使用时回读图片缩放／平移、生成中切换会话与界面、草稿预览和相关视觉状态。不得用覆盖闭环替代这层。
2. **真实 Provider 验收（需用户合法配置和明确发送）：** 分模型记录首字／终态耗时、reasoning／output Token、联网状态、工具终态、实际账单与失败恢复；不自动重试或换模型掩盖超时。
3. **Desktop 差异审计：** 只有用户要求跨端同步时，才按入口审计逐项确认 Android 新增能力在 Desktop 的去留、实现与证据，不把 Android checkpoint 外推到 Desktop。
4. **后续新增或退役模型固定门：** 新增模型必须同时检查中央目录、Provider／凭据、手动与 Auto 顺序、附件材料桥、联网／推理参数、调用账本、费用诚实边界、设置、错误隔离和完整回归；退役模型必须保留历史归因且在请求边界失败关闭，任一缺失都不得写成“完整接入”。

### E. 后续复查与冲突裁决

1. 每次复查先读取当前 HEAD、工作树、`AGENTS.md` 和本节；未提交并行工作不得混入结论。
2. 只按受影响领域再读取一份现行合同和 `CURRENT_HANDOFF.md` 顶部，不从历史段落拼凑当前产品。
3. 已完成范围默认只做防回退验证；只有代码、合同或真实证据显示不一致时才重开实现。
4. 冲突顺序固定为：当前用户指令 → 当前代码与可复现证据 → 现行领域合同 → 本总控门 → 历史文档／截图／记忆。
5. 版本、Schema、测试、APK、设备或外部服务变化时更新 `CURRENT_HANDOFF.md` 与本节；行为变化更新对应合同；长期 owner 变化才更新 `decision-log.md`。禁止把同一规则复制到多个入口。

## 2026-08-28 历史总控门：checkpoint、导入链、验证与冲突裁决（已被 2026-08-31 当前门取代）

**本节仅保留 2026-08-28 当时的稳定 checkpoint 与附件恢复证据。** 其中 `c1c9ae0`、Schema 56、819 项 JVM、旧 APK／OPPO 和“当前剩余任务”均已被上方 2026-08-31 总控门取代；只可用于追溯根因，不得继续作为排程入口。
...
## 2026-08-27 历史总控门：合同收口、已验证增量与未发布代码分层（当前门为 2026-08-30）

本节只保留 2026-08-27 当时的合同路由和验证分层证据，不再是排程入口。其中 `796 / 60`、旧 Release 状态和“附件待验收”等描述先被 2026-08-28 历史门取代，现统一服从上方 2026-08-31 当前总控门；三份当前合同的路由依然有效。
...
## 2026-08-26 历史总控门（后由 2026-08-28 历史门取代；当前门为 2026-08-30）

本节仅保留 2026-08-26 当时的排程、入口和设备证据。其中自称“当前”的子标题、表头或描述都是当时语境，不得当作当前排程、当前 APK、当前测试数或当前设备状态。当前只读本文顶部 2026-08-30 总控门及它所路由的合同与交接。
