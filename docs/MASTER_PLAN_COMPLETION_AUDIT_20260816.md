...
| P5 备份/迁移等真实 Android 链 | 本地合同和既有模拟器证据存在；运行时备份/隐私/Eval owner 已改用当前 `BuildConfig.VERSION_NAME`，不再把新包元数据写成历史 `0.3.0-p5d` / `0.3.0-p4m`；P5-D manifest/preflight 从当前打开的 Room 读取 Schema（现为 37），恢复前也核验候选 SQLite Schema，完整 1→37 迁移链受定向测试保护 | 自动化/历史模拟器 + 当前定向合同 + Debug 构建 | 当前签名构建可产出；仍缺以当前隔离包完成的 SAF 成功导出、受控恢复和冷启动真实链，不能由旧包替代 |
