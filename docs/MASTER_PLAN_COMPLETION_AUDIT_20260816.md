# 南枫 AI 总控方案需求—证据完成审计（2026-08-16；2026-08-28 当前排程更新）

## 2026-08-28 当前总控补充：ChatGPT 累积 ZIP 的安全合并已获真实包 JVM 证据

本节优先于紧随其后的 2026-08-27 当前门中关于 P6-K 的旧“同源不同 hash 冲突”表述；其余合同优先级不变。P6-K 的唯一业务正文是 [ChatGPT／Claude ZIP 导入采纳合同](P6K_CHATGPT_CLAUDE_ZIP_IMPORT_ADOPTION_CONTRACT.md)，具体包、测试和性能事实只读 [当前交接](CURRENT_HANDOFF.md) 顶部。
...
## 2026-08-27 当前总控门：合同收口、已验证增量与未发布代码分层

本节是本文件唯一的**当前排程入口**，优先于后文全部 2026-08-16 至 2026-08-26 的审计、旧 APK／设备哈希、旧“当前”描述、旧 UI 数值及 P4 阶段措辞。后文保留为发生时的证据，不能用于推翻当前代码、当前合同或下一步。上一节标题中的“2026-08-26 当前总控门”自本节写入后仅保留为历史快照。
...
## 2026-08-26 当前总控门（历史快照；已被 2026-08-27 当前总控门取代）

本节是本文件的**当前排程入口**，优先于后文所有 2026-08-16 至 2026-08-23 的历史审计、旧 APK 哈希、旧“未覆盖安装”描述、旧会话 UI 数值，以及旧 P4 的“显式选择／不自动进入 Context”阶段措辞。历史段落保留其发生时的证据价值，不可反向改写当前代码、当前合同或下一步。
