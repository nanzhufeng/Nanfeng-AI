...
- 普通设置只展示经过验证的“旗舰 / 均衡 / 快速”等可理解预设和能力说明。
...
- 工作记忆：当前任务和近期状态，随上下文压缩。
- 会话记忆：当前 Conversation 内可复用摘要。
- 长期记忆：用户可见、可编辑、可删除、可暂停、可撤销、带来源和适用范围。
- 默认不自动保存高敏信息、第三方内容、外部网页指令或工具结果。
- 自动发现的长期记忆先显示确认卡，未确认不进入长期记忆。
- 冲突记忆并存时提示用户选择或更新，不静默覆盖。
- 长期记忆公开支持新增、搜索、查看、修改、删除、暂停、批量删除和历史；任何“系统自动管理”都不能隐藏这些用户权利。
- 每条记忆记录来源、适用范围、版本、状态、最近确认时间和变更历史；Project、会话与全局范围不得混用。
...
导航方案在视觉设计阶段用真实页面数量、频率和目标设备验证；不能因早期原型方便就永久锁定。
...
- 以南枫自适应工作台负责结构、密度、状态与响应式；以南枫 UI 体系负责皮肤、材质和组件几何。
- 主内容卡保持纯白，页面底用中性低饱和灰或灰绿，状态色只服务语义。
- 可交互表面、阴影、按压、水波纹、悬停和焦点服从同一轮廓。
- App 自有 Dialog、DropdownMenu 和 Popup 内容面为纯白，灰色只用于外部遮罩。
- 保存、复制、删除和提交保留清晰文字；低后果尾部导航可用明确图标。
- UI 设计通过、模拟器通过、真机视觉通过和功能链路通过必须分开报告。
...
**P5-D 手工本地备份、恢复、迁移与正式交付基线（本地实现完成；P5 未退出）**：独立合同为 `P5D_LOCAL_BACKUP_RESTORE_DELIVERY_CONTRACT.md`。用户手工 SAF 导出 `.nfai-backup`，由一致性 SQLite `VACUUM INTO` 快照（不是 WAL/SHM 复制）、版本化 Manifest、受控资产、逐项/Manifest hash 与 SAF 回读组成；高敏/secret、zip-slip/symlink、重复 entry、未知版本、超限/压缩炸弹和 hash 异常整体拒绝。恢复先隔离预检，非空仅显式替换或取消，替换有 checkpoint/staging/rollback 与完全重启边界，不做 merge 或自动重放。Android Auto Backup 和 device transfer 均保持禁止。`0.3.0-p5d`/code39 Debug/Release 交付、v2/v3/证书/本地 SHA256SUMS 已冻结；API35 `emulator-5554` 同签名 code39 覆盖与 base.apk 回拉已完成。模拟器历史数据触发高敏总拒绝，因此 SAF 成功导出/替换和真实旧 APK 16→17 覆盖迁移仍独立待验证；不以自动合同、旧 APK 签名或新包安装替代。详见 current handoff。
