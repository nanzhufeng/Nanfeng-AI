...
## 当前 Desktop 预览与层级（2026-09-10）

- 图片、PDF、视频、音频、文本和不支持格式提示共用应用视口全屏层；加载／失败同样占满窗口。标题和关闭固定，正文获得剩余空间，图片默认完整适配，长文本内部滚动；不再被侧栏或分割线覆盖。这是应用内容区全屏，不强制切换 macOS 系统全屏空间。
- PDF 经原有 workspace／附件 ID、字节数与 SHA 校验后，由 macOS CoreGraphics 本机绘制当前页 PNG，最长边 2048px。保留总页数、上一页／下一页和持久页码；不执行 PDF 动作，不放宽 `frame-src 'none'`／`object-src 'none'`。其他桌面平台未接入绘制时明确报错。原件不改写；当前页面是栅格，未提供 PDF 文本选择。
- 全局弹窗隔离背景点击、键盘焦点和应用快捷键；Escape 只关闭顶层，关闭后恢复背景。滚动容器内的生命周期／记忆菜单使用原生 popover，并按触发按钮与窗口边缘定位。
- UTF-8 文本读取既有 128 KiB 上限、音视频显式播放及私有附件 owner 边界保持；不能预览的格式显示真实原因，不宣称所有格式都可阅读。
- 代码与验证依据：[本轮记录](review/20260910/PREVIEW_LAYER_FIXES.md)。本文上方阶段状态及旧“音频不设第二层预览”是历史计划，Desktop 当前实现以本节与实际代码为准；Android 继续由当前 Android UI 合同管理。
