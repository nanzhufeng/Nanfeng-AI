...
- 云端列表读取不以迁移上传为前置；完整成员清单与恢复成功子集分别表达。删除意图持久化，后台续同步不重建远端已删除文档；重新创建须明确手动同步。旧客户端不具备防复活保证，故双端一起升级。大库耗时与真实多端链路仍单列待验收。
- 用户要求必须覆盖并说明反对未知密码反复提示。本次沿用原 Apple Development 身份完成签名，未输入密码、导出私钥或修改 ACL；不换身份、不伪装为无需私钥，不将一次成功变成永久无提示承诺。Android 同证书保数据覆盖，Desktop 同身份原位替换并保留备份；安装及正文快照证据见 [当前交接](CURRENT_HANDOFF.md)。
- 本次 checkpoint 保存当前实现和已披露的失败，不宣称全量通过；不触发真实云端标题修复／删除来补测试，不把用户数据当夹具。签名凭据政策未永久改变。
...
## 决策：模型目录名称按响应式空间显示，不以全局缩写替代事实（2026-09-13，已确定）

- 当前选择：模型选择控件从唯一目录的 `displayName` 读取名称。Desktop 和 Android 内屏／展开宽度显示完整名称；Android 外屏为保护输入区才使用紧凑名称。模型选择面的触控／鼠标命中区与其可见胶囊同步扩展，不能让完整文字脱离表面或被独立箭头、伪标签取代。
- 固定边界：助手消息页脚的紧凑归因仍服务于高密度历史阅读，不能反向覆盖 Composer 的当前选择事实；模型 ID、Provider、费用、路由和历史消息均不因显示规则改写。名称、宽度阈值和当前目录均由各端已有目录／布局 owner 管理，禁止按厂商散落字符串替换。
- 不采用方案：不将 `compactModelName`／`composerModelShortNameForUser` 扩大为全局展示规则；不为每个模型手写宽度，也不把外屏强行扩宽至挤压草稿输入区。

## 决策：启动恢复依据退出身份，整篇摘要保存检查修订（2026-09-10，已确定）

- 当前选择：恢复使用实际退出 ID 与有效状态；全文摘要替换在事务内检查编辑起点修订集合，失败保留草稿。
- 依据与取舍：排序首项不代表用户退出位置；无修订检查的整篇覆盖会丢失编辑期间新增内容。保留旧记录与明确失败，比隐式选择和覆盖更可追溯。
- 重新评估条件：多窗口恢复或协同编辑引入新的身份／冲突语义。当前验收层级只读交接，不以单测代替真实升级。

## 决策：设置保存使用字段 patch 与单一写入队列（2026-09-10，已确定）

- 当前选择：读取最新 revision，只合并本次变更字段；全屏保存失败不关闭，独立设置 owner 分别验证重开。
- 依据与取舍：旧页面快照全量覆盖会抹去快速连续修改。串行 patch 保留各次用户意图，不用延长防抖冒充正确持久化。
- 重新评估条件：跨进程并发或云端双向合并加入后，重新审查 revision 冲突与事务边界。

## 决策：Desktop 本机备份按业务语义覆盖分库，设备状态与派生索引不随包覆盖（2026-08-31）

- 当前选择：Desktop 主工作区 SQLite、独立费用账本 SQLite 与受控私有资产进入同一 `.nfai-backup`，两个 SQLite 都用一致性快照而不是复制活动数据库文件。Provider／模型路由、同步设备元数据与窗口布局继续保留当前设备值；搜索和预览缓存不进包，完全重启后从已恢复 owner 重建。
- 不采用方案：不因 Desktop 物理分库而漏掉费用事实，不把 Key、恢复材料、设备路由、诊断或派生缓存当业务备份，也不在恢复当前进程继续持有旧 SQLite／页面引用。
- 重新评估条件：新增持久业务 owner 或拆分新的业务数据库时，必须先加入备份清单、预检、checkpoint、切换、回滚和往返测试；只加 UI 行不构成支持。

## 决策：退役模型保留历史归因，但在请求边界失败关闭（2026-08-31）

- 当前选择：已从用户选择、Auto 候选和设置目录撤下的模型仍保留稳定逻辑 ID，仅用于读取历史消息与识别旧会话／旧默认；普通聊天发送和重试须在读取 Key、准备附件、构造请求或外发正文之前以 `MODEL_NOT_FOUND` 结束。
- 不采用方案：不为兼容旧会话静默替换为近似模型、Auto 或 Provider 默认，也不删除历史模型名而破坏当时的归因、费用与审计事实。
- 重新评估条件：仅在用户显式迁移某个历史会话或重新启用有已核验目录与能力合同的模型时，才新建可见迁移动作；迁移不改写旧消息。

## 决策：图片查看按真实几何溢出决定缩放与平移（2026-08-31）

- 当前选择：全屏图片初始保持完整内容可见的合同；双指可继续缩小至整图完全落入视口，双击以触点为中心放大，单指在图片任一方向真实溢出时可连续查看，最终只按内容边界收束。
- 不采用方案：不把“必须先大于固定缩放值”作为拖动门槛，不为纵图或横图固定大小／位置，也不以弹层滚动或手势竞争替代内容边界判断。
- 重新评估条件：若新增裁切、旋转、超大图分片或多图分页，须沿用同一可逆几何模型，并补充分辨率、方向和无障碍的回归合同。
...
## 决策：Desktop Compare 作为单入口产品例外复用普通聊天执行链（2026-09-01）

- 当前选择：Android 公开模型选择面继续隐藏 Compare；Desktop 只保留 Composer 一个“对比”入口，固定 OpenRouter GPT-5.6 Terra 与 Claude Sonnet 5。一次点击原子写入一个 USER、两个 sibling Assistant、两个普通聊天 Attempt 和一个 content-free Compare envelope。
- 已确认事实与依据：用户明确要求 Desktop 直接执行且不增加第二确认面；现有普通聊天已经拥有固定 endpoint、Security.framework scoped credential、上下文／附件投影、流式传输、Usage Ledger、失败／UNKNOWN、停止与重试语义，另建 transport／credential／正文 owner 会形成平行真值。
- 关键推理与权衡：Compare 只增加双分支编排与 lineage，继续复用普通聊天 transport 和 message tree；每支独立记账、失败和显式重试，整个 execution 共用停止。打包环境用只读 workspace 轮询补足流式 UI，不开启全局 Tauri API，也不在用户切换会话后抢回页面。
- 放弃方案及原因：删除 JS fail-closed owner 与 mock-only Rust owner；不保留模型菜单或长按重复入口；不 fallback Provider／模型；不存正文、Prompt、Key、路径、附件字节或原始 payload 到 Compare 表；不把 localhost mock 写成真实 OpenRouter 已验。
- 风险与待验证项：真实 OpenRouter／Key／计费回读未做；原生 PARTIAL 与双 Stop 已实看，但 Computer Use 未稳定闭环点击后的双 `CANCELLED`；系统退出后按 UNKNOWN 恢复，不提供 Android 前台服务同级的持续生成。
- 重新评估触发条件：用户要求改变 Desktop 单入口、固定逻辑模型、Provider、分支上限或综合答案；或真实 Provider 证明模型映射／费用字段／停止语义不满足当前合同。

## 决策：对话风格采用设置默认、会话覆盖与回答级冻结事实（2026-09-01，已确定）

- 当前选择：设置中用户当前选定的“基础风格和语气”是普通对话的全局默认；当前会话可以保存独立显式覆盖，优先级高于设置且只作用于后续请求。成功落库的 Assistant 回复永久保存生成时实际风格和实际联网结果，旧回答缺字段时保持未知。
- 已确认事实与依据：用户明确要求当前对话可反复快速切换、不得改变总开关或其他对话，并纠正“新安装默认直言不讳”为“默认应取设置里选定风格”。当前会话、设置与运行时合同以及 Schema 65 迁移、Room 往返和请求序列化测试已经形成一致 owner。
- 关键推理与权衡：用“覆盖可缺省”表达继承关系，才能区分用户选择中性“默认”和从未覆盖；把回答执行证据绑定到既有消息归因，才能避免之后的设置／开关反推历史。只有可验证 HTTP(S) 来源可证明实际联网，配置状态和模型自述均不够。
- 放弃方案及原因：不让会话快捷选择覆盖设置全局值；不把 App 初始中性回退误写成用户全局默认；不按当前开关、当前风格或当前模型倒填旧回答；不把 Flash／Plus／Pro 等档位短名用于路由身份。
- 风险与待验证项：真实 Provider 下的回答级联网与风格语义仍需合法凭据和非敏感输入单独验收；旧回答只能显示未记录，不能补造。当前短模型名可能合并同系列不同档位，因此只允许出现在空间受限的显示面。
- 重新评估触发条件：用户要求会话风格同步到其他对话／设备、Provider 返回新的来源结构、回答分支需要独立风格继承，或短名无法区分用户必须直接选择的同版本模型。

## 决策：冻结已披露接收方，并在原 owner 内修复存储／读取边界（2026-09-10）

- 依据：`4bf3e00` 的四项[边界发现](review/20260910/BOUNDARY_REVIEW.md)与本轮[行为验证](review/20260910/BOUNDARY_FIXES.md)。
- Android 把 Auto 可用性筛选移到本地预览，授权由实际渲染的发送入口生成并绑定 API 模型 ID；保留一次点击发送，拒绝通过排队后的模型重算或只禁用按钮来充当授权冻结。目录变化后明确失败，优先保持用户披露边界。
- Desktop 在锁内恢复／发布目录，已配置缺库失败关闭；为保留合法备份中断恢复，复用原 checkpoint owner，恢复后再校验，不创建空数据库来掩盖丢失。
- 头像对未知长度逐块限额并取消超限流；不依赖 Content-Length，不先 arrayBuffer 后检查。读到的单个传输块及最终拼接仍占内存，所以不宣称进程硬内存上限。
- 本轮不改 Room／SQLite schema；旧 v1 发送审计继续可读，新发送要求 v2 快照。原生和真实服务验收仍独立记录。


## 2026-09-10：DeepSeek V4.1 Flash 使用新 API，保留持久化选择身份

将 Android／Desktop 当前 Flash 映射到官方 `deepseek-flash`，保留 `DEEPSEEK_V4_FLASH` 持久键，避免丢失已选模型、启用设置与历史引用。缓存和当前 Composer 投影显式升级，历史消息／调用账本不重命名；新模型原生图片和版本化计价贯通实际适配层。官方事实、冲突、验证及未来 Pro 转发边界见 [升级记录](review/20260910/DEEPSEEK_V41_UPGRADE.md)。


## 2026-09-18：取消恢复码与端到端加密，统一账号直同步

- 依据：用户明确确认“按此方案，取消恢复码和端到端加密”，接受正文可由服务端读取。覆盖 2026-09-15 加密写入方案；旧记录不删除。
- 裁决：唯一当前合同为 `P7F_SELECTED_CONVERSATION_SYNC_CONTRACT.md`。新写入 direct，认证／HTTPS／账号隔离与 CAS 保留；旧密文只由具备对应材料的设备验证后转换。移除 UI 与 Tauri 可调用恢复码操作，保留历史解码能力以避免数据不可恢复。
- 取舍：简化日常和新设备使用；不能凭空解开不同设备材料产生的历史密文。迁移未完成前，缺材料设备明确报告待处理，其他记录继续读取。
- 服务部署、双端安装及真实回读以当前交接的证据为准，源码修改不代表已上线。

## 2026-09-18：恢复码最终用途澄清与账号直同步去依赖

用户先要求保留恢复码管理，随后明确“如果以后完全用不到则可以删除，不能入口消失后又被恢复码卡住”。最终按 Google 登录即可日常同步执行，修复正常登录仍初始化／检查旧密钥的遗漏；不恢复恢复码 UI，不重新加密云端会话。旧材料只用于逐条兼容迁移并保留，不能阻断 direct 文档。双端读取按钮加长、文字居中、删除说明小字。长期门禁见 AGENTS 和 P7-F 合同。
