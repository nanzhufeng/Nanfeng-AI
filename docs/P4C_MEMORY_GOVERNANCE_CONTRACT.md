...
> **历史阶段定位（2026-08-26）：** 本文记录当时尚未接入普通发送链的 P4-C 本机治理边界。它继续约束 Memory 的显式创建、状态、范围与敏感内容门禁；其中“不会自动加入对话上下文”等表述**不能**覆盖当前普通聊天。现行普通聊天的相关性检索与外发最小化边界只见 [Android 当前运行时上下文合同](ANDROID_RUNTIME_CONTEXT_CURRENT_CONTRACT.md)。
