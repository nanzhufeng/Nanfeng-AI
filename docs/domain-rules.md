...
> **当前普通聊天覆盖说明（2026-08-26）：** 下文 P4 阶段中“Memory／Knowledge 不自动进入 Context、只经显式选择、不构造 Prompt”的描述，保留其当时独立本机治理或预览流程；它们不适用于已经接通的普通发送链。普通聊天的用户资料注入、已开启 Memory／资料库的本机相关性检索与最小外发，只以 [Android 当前运行时上下文合同](ANDROID_RUNTIME_CONTEXT_CURRENT_CONTRACT.md) 为准。
...
1. 周期监控必须由用户在“已计划”表单明确创建；从对话末尾进入时只可预填本机来源，禁止自动发送或解析会话正文、附件和 Context。
2. 每次运行只可发送用户在任务表单中写下的任务名称与监控要求，并记录实际网络接收服务商、模型、可用 Token、结果或安全错误码。
3. 任务暂停或删除后不得再联网；每个任务同一时刻至多保留一条受网络约束的后台请求。
4. 通知只提示本次任务完成，不携带结果正文；Android 通知权限未授权时，结果仍只保留在本机任务页。
5. 默认模型为 GPT-5.6 Terra 标准档，不得隐式升级到 Pro；服务、凭据、模型或官方检索能力不满足时必须保存可理解的失败状态，不能伪造简报。
