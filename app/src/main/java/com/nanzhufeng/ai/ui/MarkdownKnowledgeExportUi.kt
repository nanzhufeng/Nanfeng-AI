...
@Composable fun MarkdownKnowledgeExportCard(onOpen: () -> Unit) = WhiteCard { Text("Markdown 导出", style = androidx.compose.material3.MaterialTheme.typography.titleMedium, fontWeight = FontWeight.SemiBold); Spacer(Modifier.height(6.dp)); Text("先明确选择正式活动 Knowledge，再以可回读 Markdown 与 Manifest 导出。", color = SecondaryText, style = androidx.compose.material3.MaterialTheme.typography.bodySmall); Spacer(Modifier.height(12.dp)); Button(onClick = onOpen, modifier = Modifier.fillMaxWidth().height(48.dp), shape = RoundedCornerShape(16.dp), colors = ButtonDefaults.buttonColors(containerColor = BrandGreen)) { androidx.compose.material3.Icon(Icons.Rounded.FileDownload, null); Text("选择范围并导出") } }
@Composable fun MarkdownKnowledgeExportDialog(state: MarkdownExportUiState, onDismiss: () -> Unit, onToggle: (KnowledgeItemId) -> Unit, onExport: () -> Unit) = AlertDialog(onDismissRequest = onDismiss, containerColor = ForegroundSurface, shape = RoundedCornerShape(24.dp), title = { Text("导出 Markdown Knowledge", fontWeight = FontWeight.SemiBold) }, text = { Column(Modifier.heightIn(max = 500.dp).verticalScroll(rememberScrollState())) { Text("仅导出所选的活动知识。", color = SecondaryText, style = androidx.compose.material3.MaterialTheme.typography.bodySmall); state.entries.forEach { item -> androidx.compose.foundation.layout.Row(verticalAlignment = Alignment.CenterVertically) { Checkbox(item.id in state.selected, { onToggle(item.id) }); Column { Text(item.title, fontWeight = FontWeight.Medium); Text(item.tags.joinToString(" · ").ifBlank { "无标签" }, color = SecondaryText, style = androidx.compose.material3.MaterialTheme.typography.bodySmall) } } }; state.result?.let { Text("已导出：${it.fileName}", color = BrandGreen, style = androidx.compose.material3.MaterialTheme.typography.bodySmall) }; state.message?.let { Text(it, color = ErrorRed, style = androidx.compose.material3.MaterialTheme.typography.bodySmall) } } }, dismissButton = { TextButton(onClick = onDismiss) { Text("关闭") } }, confirmButton = { Button(onClick = onExport, enabled = !state.working && state.selected.isNotEmpty()) { Text("导出 ${state.selected.size} 项") } })
