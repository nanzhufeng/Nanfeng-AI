...
class AndroidContextSelectionAuditStore(context: Context) : ContextSelectionAuditStore {
    ...
    private fun toJson(record: ContextSelectionAuditRecord) = JSONObject().apply {
        put("createdAt", record.createdAt.toString()); putOpt("conversationId", record.conversationId); put("providerId", record.providerId.name); put("modelId", record.modelId); put("tokenizerId", record.tokenizerId)
        ...
        put("sources", JSONArray(record.selectedSources.map { source -> JSONObject().apply { put("kind", source.kind); put("id", source.stableId); put("title", source.title.take(120)); put("tokens", source.estimatedTokens) } }))
    }
    ...
    private fun fromJson(value: JSONObject): ContextSelectionAuditRecord {
        ...
        return ContextSelectionAuditRecord(
            createdAt = Instant.parse(value.getString("createdAt")), conversationId = value.optString("conversationId").takeIf { it.isNotBlank() }, providerId = ProviderId.valueOf(value.getString("providerId")), modelId = value.getString("modelId"), tokenizerId = value.getString("tokenizerId"),
            ...
            selectedSources = buildList { for (index in 0 until sources.length()) { val source = sources.getJSONObject(index); add(ContextSelectionSource(source.getString("kind"), source.getString("id"), source.getString("title"), source.getInt("tokens"))) } },
    ...
    private companion object { const val MAX_ENTRIES = 20 }
}
