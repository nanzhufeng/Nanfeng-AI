...
class AndroidContextSelectionAuditStore(context: Context) : ContextSelectionAuditStore {
    ...
    private fun toJson(record: ContextSelectionAuditRecord) = JSONObject().apply {
        put("createdAt", record.createdAt.toString()); put("providerId", record.providerId.name); put("modelId", record.modelId); put("tokenizerId", record.tokenizerId)
    ...
    private fun fromJson(value: JSONObject): ContextSelectionAuditRecord {
        ...
        return ContextSelectionAuditRecord(
            createdAt = Instant.parse(value.getString("createdAt")), providerId = ProviderId.valueOf(value.getString("providerId")), modelId = value.getString("modelId"), tokenizerId = value.getString("tokenizerId"),
