...
class RoomP6KZipImportTaskRepository(private val database: NanfengAiDatabase) : com.nanzhufeng.ai.domain.P6KZipImportTaskRepository {
    ...
    override fun find(id: com.nanzhufeng.ai.domain.P6KZipTaskId): com.nanzhufeng.ai.domain.P6KZipImportTask? = database.p6kZipImportTaskDao().task(id.value)?.toP6KZipDomain(database.p6kZipImportTaskDao(), database.p6kImportIdentityLedgerDao().batchReceipt(id.value))
    override fun list(): List<com.nanzhufeng.ai.domain.P6KZipImportTask> = database.p6kZipImportTaskDao().all().map { it.toP6KZipDomain(database.p6kZipImportTaskDao(), database.p6kImportIdentityLedgerDao().batchReceipt(it.id)) }
...
private fun P6KZipImportTaskEntity.toP6KZipDomain(dao: P6KZipImportTaskDao, receipt: P6KImportBatchReceiptEntity?): com.nanzhufeng.ai.domain.P6KZipImportTask {
...
internal fun com.nanzhufeng.ai.domain.P6KZipAssetRecoveryJob.toEntity() = P6KZipAssetRecoveryJobEntity(
    ...
    unattributedCandidates, sourceReferenceRecords, fallbackNamedAssets, inferredGeneratedImages,
    originLinkedLibraryImages, inferredLibraryImages,
...
internal fun P6KZipAssetRecoveryJobEntity.toDomain() = com.nanzhufeng.ai.domain.P6KZipAssetRecoveryJob(
    ...
    sourceReferenceRecords, fallbackNamedAssets, inferredGeneratedImages,
    originLinkedLibraryImages, inferredLibraryImages,
