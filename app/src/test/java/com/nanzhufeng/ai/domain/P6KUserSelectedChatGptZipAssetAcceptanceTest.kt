...
class P6KUserSelectedChatGptZipAssetAcceptanceTest {
    ...
    fun `selected official export maps every source-owned current-path asset without guessing`() {
        ...
        assertEquals(mapOf("image" to 1416, "file" to 118, "video" to 75, "audio" to 6), uniqueAssetClassCounts)
        assertEquals(mapOf("image" to 1417, "file" to 118, "video" to 75, "audio" to 6), occurrenceClassCounts)
        ...
        assertEquals(1617, sourceReferenceRecords)
        assertEquals(1617, occurrences.size)
        assertEquals(1, referenced.minus(mapping.assets.keys).size)
        assertEquals(1615, mapping.assets.size)
        assertEquals(60, candidates.size - mapping.assets.size)
