...
class CnyMoneyDisplayTest {
    @Test fun `USD micros are converted with the versioned current reference rate`() {
        assertEquals("¥0.035013", CnyMoneyDisplay.label(5_210L, "USD", estimated = false))
        assertEquals("¥0.035886", CnyMoneyDisplay.label(5_340L, "USD", estimated = true))
    }
    ...
    @Test fun `CNY micros keep their original value without a conversion qualifier`() {
        ...
        assertEquals("¥1.25", CnyMoneyDisplay.label(1_250_000L, "CNY", estimated = true))
    }
    ...
    @Test fun `mixed supported currencies are converted before aggregation`() {
        ...
        assertEquals("¥7.720309", total)
    }

    @Test fun `summary amounts are concise RMB without a source suffix`() {
