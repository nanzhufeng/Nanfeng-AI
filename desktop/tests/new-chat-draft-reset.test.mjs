...
test('new chat has one native draft owner and never revives browser-cache text', async () => {
  ...
  const reset = source.slice(source.indexOf('function resetNewConversationComposer()'), source.indexOf('function scheduleOrdinaryComposerDraft()'));
  assert.match(reset, /loadOrdinaryComposerDraft\(\)/);
  assert.doesNotMatch(reset, /persistOrdinaryComposerDraft/);
  assert.match(composerRouteOwner, /if \(action === 'new-chat'\)[\s\S]{0,300}resetNewConversationComposer\(\)/);
  assert.match(composerRouteOwner, /loadOrdinaryComposerDraft\(\)\.then\(render\)/);
