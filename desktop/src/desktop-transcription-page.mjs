...
export function renderDesktopTranscriptionPage({ projection = TRANSCRIPTION_PREVIEW_STATE, selectedTaskId = null, native = false, busyTaskId = null, previewInteractive = false, evidenceLabel = '' } = {}) {
  ...
  return `<section class="canvas transcription-page" aria-label="南枫转写">${pageHeader()}${pageBody}<footer class="transcription-page-footer"><button class="primary transcription-pick" data-action="pick-transcription-document" ${pickEnabled ? '' : 'disabled'}>${icon(icons.import, '选择文件')}<span>选择图片或 PDF</span></button></footer></section>`;
}
