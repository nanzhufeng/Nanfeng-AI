...
function accountSyncPage(context) {
  ...
  const recoveryCard = recovery?.setup ? `<section class="android-settings-card android-account-recovery" aria-label="保存恢复码"><strong>保存恢复码</strong><p>这是恢复加密同步的唯一凭据；不会上传到云端。</p><code class="android-account-recovery-code">${escapeHtml(recovery.recoveryCode)}</code><button data-action="copy-account-recovery-code" data-copy-action data-copy-value="${escapeHtml(recovery.recoveryCode)}">${icon(icons.copy, '复制恢复码')}<span>复制恢复码</span></button><label class="android-settings-check"><input id="account-recovery-saved" type="checkbox">我已在安全位置保存</label><button class="primary" data-action="confirm-account-recovery-code">确认并启用端到端加密同步</button></section>` : recovery?.rotation ? `<section class="android-settings-card android-account-recovery" aria-label="更换恢复码"><strong>更换恢复码</strong><label class="android-settings-field"><span>新恢复码</span><input id="new-recovery-code" type="password" autocomplete="new-password" minlength="12" maxlength="128" placeholder="至少 12 个字符"></label><button class="primary" data-action="confirm-account-recovery-rotation">确认更换</button></section>` : '';
  const recoveryAndCloud = signedIn ? `<section class="android-settings-card android-account-cloud-restore android-account-phone-recovery"><strong>恢复与安全</strong>${account.state === 'AUTHENTICATED_NEEDS_RECOVERY_CONFIRMATION' ? '<p>新账号创建恢复码；已有云端对话请输入原恢复码接入。</p><label class="android-settings-field"><span>已有恢复码</span><input id="existing-account-recovery-code" type="password" autocomplete="off" minlength="12" maxlength="128" placeholder="至少 12 个字符"></label><button data-action="recover-account-existing-recovery">使用已有恢复码接入</button><button class="primary" data-action="create-account-recovery-code">创建新的恢复码</button>' : '<button data-action="create-account-recovery-rotation">更换恢复码 / 已丢失</button><button data-action="load-account-cloud-documents">'+icon(icons.cloudDownloadSolid, '读取云端列表')+'<span>读取云端列表</span></button>'}</section>` : '';
