...
export const SETTINGS_REGISTRY = Object.freeze([
  ...
  { id: 'local-control', group: '基础', title: '更多本地控制面', description: '进入已有本地工作区，不触发 Provider 或外部访问。', scope: 'device', syncPolicy: 'local-only', risk: 'normal' },
...
function settingsCenterCanvas(dualPath, status, data, native, p6eAcceptance = { enabled: false, receipt: null }, p6gCatalog = null, p6gGlobalDefault = null, section = 'data', search = '', p6kTask = null, p6kLink = {}, v2CommittedExchanges = globalThis.__nanfengV2CommittedExchanges || []) {
  ...
  const v2WorkspaceExchangeRow = `<section class="settings-section"><h2>完整工作区交换（v2）</h2><article class="settings-row"><div><strong>私有 v2 导入</strong><p>仅读取您选择的 .nfai-exchange，或 Android DocumentsUI 追加的 .nfai-exchange.zip；文件名不替代严格预检，只有 v2 manifest、语义与字段哈希一致才写入独立私有归档与回执，不合并、覆盖或恢复为当前 Desktop 工作区。</p><small>device · local-only · private archive · content-free receipt</small></div><div class="settings-row-controls"><button data-action="select-v2-workspace-exchange" ${native ? '' : 'disabled'}>选择 v2 交换包</button></div></article></section>`;
  ...
  featureReviewContent = featureReviewContent.replace('</section>', `<article class="settings-row"><div><strong>完整工作区交换（v2）</strong><p>当前：待您判断是否保留；Android 可从设置 → 数据与导入显式选择完整范围并经系统保存位置导出 v2 包，Desktop 仅可从设置选择 v2 包后私有导入。</p><small>建议：只保留双端设置二级入口，不增加聊天、Composer 或工作页常驻按键；它不是备份、云同步，也不表示已恢复为原生对象。</small></div></article></section>`);
  ...
  const localControlContent = `<p class="settings-page-intro">只进入已有的本地工作区；这里不创建对象、不读凭据、不调用 Provider，也不发起外部访问。</p><section class="settings-section"><h2>更多本地控制面</h2><article class="settings-row"><div><strong>本地工作区</strong><p>Projects、知识与关系、长期 Memory 分别继续使用已有 Rust SQLite owner；进入后所有写入仍由其现有表单和事务处理。</p><small>device · local-only · reversible navigation</small></div><div class="settings-row-controls"><button data-action="show-projects">Projects</button><button data-action="show-knowledge">知识与关系</button><button data-action="show-memory">长期 Memory</button></div></article><article class="settings-row"><div><strong>边界</strong><p>这些入口不会自动加入对话上下文，也不代表联网、同步、Provider 或自动执行已可用。</p><small>no credential · no provider · no external access</small></div></article></section>`;
  featureReviewContent = featureReviewContent.replace('Desktop 仅可从设置选择 v2 包后私有导入。', 'Desktop 仅可从设置选择 v2 包私有导入，或从已提交私有记录经系统保存位置回导。');
  featureReviewContent = featureReviewContent.replace('</section>', `<article class="settings-row"><div><strong>更多本地控制面</strong><p>当前：待您判断保留或删减 · 入口：设置 → 更多本地控制面。可进入已有的 Projects、知识与关系、长期 Memory 工作页。</p><small>建议：保留设置二级入口；不建议在聊天主页、Composer 或会话详情增加按键，避免把本地管理误解为发送、联网或自动执行。</small></div></article></section>`);
  const content = section === 'model' ? modelContent : section === 'ai-model-service' ? aiModelServiceContent : section === 'local-control' ? localControlContent : section === 'feature-review' ? featureReviewContent : section === 'privacy' ? privacyContent : section === 'general' ? generalContent : `${dataContent}${v2WorkspaceExchangeRow}${v2WorkspaceExchangeReexportRow}${p6jImportRow}${p6kZipRow}`;
...
export function renderChatFirstShell({
  ...
  const main = pane === 'settings' ? settingsCenterCanvas(dualPath, status, data, native, p6eAcceptance, p6gCatalog, p6gGlobalDefault, settingsSection, settingsSearch, p6kTask, p6kLink, v2CommittedExchanges) : showWorkPanel ? `
