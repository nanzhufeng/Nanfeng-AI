...
function settingsCenterCanvas(dualPath, status, data, native, p6eAcceptance = { enabled: false, receipt: null }, p6gCatalog = null, p6gGlobalDefault = null, section = 'data', search = '', p6kTask = null, p6kLink = {}) {
  ...
  featureReviewContent = featureReviewContent.replace('</section>', `<article class="settings-row"><div><strong>完整工作区交换（v2）</strong><p>当前：待您判断是否保留；Desktop 仅可从设置 → 数据与导入选择 v2 包，严格预检后保存私有归档、provenance 与回执。</p><small>建议：只保留设置二级入口，不增加聊天、Composer 或工作页常驻按键；它不是备份、云同步，也不表示已恢复为 Desktop 原生对象。</small></div></article></section>`);
