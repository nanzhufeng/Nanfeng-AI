...
function settingsCenterCanvas(dualPath, status, data, native, p6eAcceptance = { enabled: false, receipt: null }, p6gCatalog = null, p6gGlobalDefault = null, section = 'data', search = '', p6kTask = null, p6kLink = {}, v2CommittedExchanges = globalThis.__nanfengV2CommittedExchanges || []) {
  ...
  featureReviewContent = featureReviewContent.replace('</section>', `<article class="settings-row"><div><strong>批量清理已归档与回收站</strong><p>当前：Android 已在设置 → 对话管理 → 已归档 / 回收站提供批量清理；清空已归档只移入回收站，清空回收站才永久删除。Desktop 尚无同一会话生命周期 owner。</p><small>建议：仅保留 Android 两个管理页的低频入口；不在聊天主页或 Composer 增加按键。Desktop 有同一真实 owner 后再实现，避免展示无效清理操作。</small></div></article></section>`);
