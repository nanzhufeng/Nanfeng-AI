...
function settingsCenterCanvas(dualPath, status, data, native, p6eAcceptance = { enabled: false, receipt: null }, p6gCatalog = null, p6gGlobalDefault = null, section = 'data', search = '', p6kTask = null, p6kLink = {}) {
  ...
  const featureReviewContent = `<p class="settings-page-intro">新增能力先在此展示用途、现有入口和待您判断项；本页不直接启用、删除或调用功能。</p><section class="settings-section"><h2>新增功能审阅</h2><article class="settings-row"><div><strong>ChatGPT / Claude ZIP 导入</strong><p>当前：待您判断保留或删减 · 入口：设置 → 数据 → 第三方 ZIP 导入。</p><small>建议：保留设置入口；暂不在对话主页添加快捷按钮，避免高敏感导入被误触。</small></div></article><article class="settings-row"><div><strong>未关联媒体人工关联</strong><p>当前：待外部归属证据后再判断是否启用 · 入口：ZIP 导入批次详情。</p><small>建议：仅在存在未关联媒体时提供二级操作；不在聊天主界面常驻功能按钮。</small></div></article></section>`;
  ...
  const content = section === 'model' ? modelContent : section === 'feature-review' ? featureReviewContent : section === 'privacy' ? privacyContent : section === 'general' ? generalContent : `${dataContent}${p6jImportRow}${p6kZipRow}`;
