...
pub struct Source { pub user_text: String, pub assistant_text: String }
...
pub fn opening_source(conversation: &Value) -> Option<Source> {
    ...
    let source = Source { user_text: message_text(user), assistant_text: message_text(assistant) };
...
mod tests {
    ...
    #[test] fn source_requires_first_completed_pair() {
        ...
        assert_eq!(opening_source(&conversation), Some(Source { user_text:"整理 KFK".into(), assistant_text:"我会整理 KFK 资料。".into() }));
    }
}
