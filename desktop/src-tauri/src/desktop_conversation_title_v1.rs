...
pub fn opening_source(conversation: &Value) -> Option<Source> {
    ...
    let user = messages.iter().find(|message| {
        message.get("role").and_then(Value::as_str) == Some("user")
            && message.get("parentId").map_or(true, Value::is_null)
    })?;
    let user_id = user.get("id")?.as_str()?;
    let assistant = messages.iter().find(|message| {
        message.get("role").and_then(Value::as_str) == Some("assistant")
            && message.get("parentId").and_then(Value::as_str) == Some(user_id)
    ...
    let has_attachment = user
        .get("blocks")
        .and_then(Value::as_array)
        .into_iter()
        .flatten()
        .any(|block| block.get("kind").and_then(Value::as_str) == Some("ASSET_REF"));
    (!source.assistant_text.is_empty() && (!source.user_text.is_empty() || has_attachment))
        .then_some(source)
}
...
pub fn request_messages(source: &Source) -> Value {
    ...
    let assistant = source
        ...
        .take(MAX_SOURCE_CHARS)
        .collect::<String>();
    let input = if user.is_empty() {
        format!("首条用户消息仅含附件。不要读取或推断附件内容；只根据下方南枫AI开头回答生成标题：\n{assistant}")
    } else {
        format!("用户开头发言：\n{user}\n\n南枫AI开头回答：\n{assistant}")
    };
...
pub fn parse_response(raw: &str) -> Option<String> {
    ...
    if !(6..=13).contains(&count) || GENERIC_TITLES.contains(&title.as_str()) {
        return None;
    }
    title
        .split(' ')
...
mod tests {
    ...
    fn parser_matches_android_title_contract() {
        assert_eq!(
            parse_response("{\"title\":\"KFK资料整理方案\"}"),
            Some("KFK资料整理方案".into())
        );
        assert_eq!(
            parse_response("{\"title\":\"完整的整理当年KFK在豆...\"}"),
            None
        );
        assert_eq!(parse_response("{\"title\":\"更新文档\"}"), None);
    }
    #[test]
    fn source_requires_first_completed_pair() {
        ...
        assert_eq!(
            opening_source(&conversation),
            Some(Source {
                assistant_message_id: "a".into(),
                user_text: "整理 KFK".into(),
                assistant_text: "我会整理 KFK 资料。".into()
            })
        );
    }
}
