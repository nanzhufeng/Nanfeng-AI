...
const acceptanceBundleName = '南枫 AI C13 验收.app';
const acceptanceExecutable = 'nanfeng-ai-desktop-c13-acceptance';
const acceptanceIdentifier = `com.nanzhufeng.ai.desktop.c13acceptance.${process.pid}.${Date.now().toString(36)}`;
...
const acceptanceParent = await mkdtemp('/tmp/nanfeng-ai-desktop-c13-bundle.');
const acceptanceRoot = await mkdtemp('/tmp/nanfeng-ai-desktop-c13-acceptance.');
...
if (!bundleStats.isDirectory() || !executableStats.isFile() || sourceExecutableHash !== sourceExecutableHashAfter) {
  throw new Error('C13 acceptance bundle isolation verification failed');
}
...
if (!info.includes(acceptanceIdentifier) || !info.includes(acceptanceExecutable)) {
  throw new Error('C13 acceptance bundle identity verification failed');
}
