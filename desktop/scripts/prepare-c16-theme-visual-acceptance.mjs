...
const acceptanceBundleName = '南枫 AI C16 验收.app';
const acceptanceExecutable = 'nanfeng-ai-desktop-c16-visual-acceptance';
const acceptanceIdentifier = `com.nanzhufeng.ai.desktop.c16acceptance.${process.pid}.${Date.now().toString(36)}`;
...
const acceptanceParent = await mkdtemp('/tmp/nanfeng-ai-desktop-c16-bundle.');
const acceptanceRoot = await mkdtemp('/tmp/nanfeng-ai-desktop-c16-visual-acceptance.');
...
if (!bundleStats.isDirectory() || !executableStats.isFile() || sourceExecutableHash !== sourceExecutableHashAfter) {
  throw new Error('C16 acceptance bundle isolation verification failed');
}
...
if (!info.includes(acceptanceIdentifier) || !info.includes(acceptanceExecutable)) {
  throw new Error('C16 acceptance bundle identity verification failed');
}
