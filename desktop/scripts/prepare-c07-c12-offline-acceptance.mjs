...
const acceptanceBundleName = '南枫 AI C07-C12 离线验收.app';
const acceptanceExecutable = 'nanfeng-ai-desktop-c07-c12-acceptance';
const acceptanceIdentifier = `com.nanzhufeng.ai.desktop.c07c12acceptance.${process.pid}.${Date.now().toString(36)}`;
...
const acceptanceParent = await mkdtemp('/tmp/nanfeng-ai-desktop-c07-c12-bundle.');
const acceptanceRoot = await mkdtemp('/tmp/nanfeng-ai-desktop-c07-c12-acceptance.');
...
if (!bundleStats.isDirectory() || !executableStats.isFile() || sourceExecutableHash !== sourceExecutableHashAfter) {
  throw new Error('C07-C12 acceptance bundle isolation verification failed');
}
...
if (!info.includes(acceptanceIdentifier) || !info.includes(acceptanceExecutable)) {
  throw new Error('C07-C12 acceptance bundle identity verification failed');
}
