...
- 按任务读取一份现行合同：会话与搜索、设置、运行时上下文、转写、工作区隔离与同步分别由 `docs/` 对应合同定义。完整模块地图见 [开发档案](docs/南枫AI完整开发档案.md)。
...
- README 预览图必须作为仓库内独立文件以相对路径引用；永不作为 GitHub Release 附件，也不得引用 Release 下载地址中的图片。Release 只上传安装包和校验文件。
